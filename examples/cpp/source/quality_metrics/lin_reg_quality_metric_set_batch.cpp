/* file: lin_reg_quality_metric_set_batch.cpp */
/*******************************************************************************
...
int main(int argc, char *argv[])
{
    ...
    trainData = services::SharedPtr<NumericTable>(new HomogenNumericTable<double>(nFeatures, 0, NumericTable::notAllocate));
    trainDependentVariables = services::SharedPtr<NumericTable>(new HomogenNumericTable<double>(nDependentVariables, 0, NumericTable::notAllocate));
...
services::SharedPtr<NumericTable> predictResults(services::SharedPtr<NumericTable>& data)
{
...
services::SharedPtr<NumericTable> predictReducedModelResults(services::SharedPtr<NumericTable>& data)
{
    services::SharedPtr<Model> model = trainingResult->get(training::model);

    services::SharedPtr<NumericTable> betas = model->getBeta();
    ...
    services::SharedPtr<NumericTable> predictedResults = predictResults(trainData);
...
void testModelQuality()
{
    ...
    services::SharedPtr<NumericTable> predictedReducedModelResults = predictReducedModelResults(trainData);
...
void printResults()
{
    ...
    std::cout << "Quality metrics for a single beta" << std::endl;
    {
        ...
        services::SharedPtr<data_management::DataCollection> coll = result->get(single_beta::betaCovariances);
