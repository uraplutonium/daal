...
void serializeNumericTable(services::SharedPtr<NumericTable> dataTable, byte **buffer, size_t *length);
services::SharedPtr<NumericTable> deserializeNumericTable(byte *buffer, size_t size);
...
int main(int argc, char *argv[])
{
    ...
    services::SharedPtr<NumericTable> restoredDataTable = deserializeNumericTable(buffer, length);
...
services::SharedPtr<NumericTable> deserializeNumericTable(byte *buffer, size_t length)
{
    /* Create a data archive to deserialize the numeric table */
    OutputDataArchive dataArch(buffer, length);

    /* Create a numeric table object */
    services::SharedPtr<NumericTable> dataTable = services::SharedPtr<NumericTable>( new HomogenNumericTable<dataFPType>() );
