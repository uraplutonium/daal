...
void initializeNetwork()
{
    ...
    for(size_t node = 0; node < nNodes; node++)
    {
        ...
        training::Distributed<step2Master> netInit(sgdAlgorithm);
        ...
        netLocal[node] = SharedPtr<training::Distributed<step1Local> >(new training::Distributed<step1Local>(sgdAlgorithm));
        netLocal[node]->input.set(training::inputModel, trainingModel);

        /* Set the batch size for the neural network training */
        netLocal[node]->parameter.optimizationSolver = sgdAlgorithm;
    }
}
