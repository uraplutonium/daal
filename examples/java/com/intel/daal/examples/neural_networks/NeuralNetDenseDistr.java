...
class NeuralNetDenseDistr {
    ...
    private static void initializeNetwork() throws java.io.FileNotFoundException, java.io.IOException {
        ...
        for (int node = 0; node < nNodes; node++) {
            ...
            DistributedStep2Master netInit = new DistributedStep2Master(context, sgdAlgorithm);
            netLocal[node] = new DistributedStep1Local(context, sgdAlgorithm);
            /* Set the batch size for the neural network training */
            netInit.parameter.setOptimizationSolver(sgdAlgorithm);
            ...
            /* Set the batch size for the neural network training */
            netLocal[node].parameter.setOptimizationSolver(sgdAlgorithm);
        }
    }
