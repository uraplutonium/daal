...
void Decompressor<lzo>::run(byte *out, size_t outLen, size_t off)
{
    ...
    do
    {
        ...
        {
            _internalBuff = daal::services::daal_malloc(uncompressedBlockSize + addSize); //addSize - workaround for non-safe Decode
        ...
        else if (_avail_out - uncompressedBlockSize >= addSize)
        {
        ...
        {
            _internalBuff = daal::services::daal_malloc(uncompressedBlockSize + addSize);
            if (EXPECT(_internalBuff == NULL, 0))
            {
                finalizeCompression();
                this->_errors->add(services::ErrorMemoryAllocationFailed);
            ...
            daal::services::daal_memcpy_s((void *)(_next_out), tmp_avail_out, (void *)(((byte *)_internalBuff) + _internalBuffOff), tmp_avail_out);
            daal::services::daal_free(_internalBuff);
            _internalBuff = NULL;
        }
