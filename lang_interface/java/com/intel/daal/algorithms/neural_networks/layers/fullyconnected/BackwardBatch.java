...
/**
 * <a name="DAAL-CLASS-ALGORITHMS__NEURAL_NETWORKS__LAYERS__FULLYCONNECTED__BACKWARDBATCH"></a>
...
public class BackwardBatch extends com.intel.daal.algorithms.neural_networks.layers.BackwardLayer {
     ...
     * @param dim        Dimension for which forward propagation step of the fully-connected layer is performed
     */
    public BackwardBatch(DaalContext context, Class<? extends Number> cls, Method method, long nOutputs, long dim) {
        super(context);
        ...
        this.cObject = cInit(prec.getValue(), method.getValue(), nOutputs, dim);
        input = new BackwardInput(context, cGetInput(cObject, prec.getValue(), method.getValue()));
        parameter = new Parameter(context, cInitParameter(cObject, prec.getValue(), method.getValue()));
    }
    ...
    private native long cInit(int prec, int method, long nOutputs, long dim);
