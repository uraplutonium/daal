...
static void load_daal_thr_dll(void)
{
    ...
    switch(__daal_serv_get_thr_set())
    {
        case daal::services::Environment::MultiThreaded: {
            ...
            printf("DAAL FATAL ERROR: Cannot load libdaal_thread.dll.\n");
            exit(1);
        ...
        case daal::services::Environment::SingleThreaded: {
            ...
            printf("DAAL FATAL ERROR: Cannot load libdaal_sequential.dll.\n");
            exit(1);
        ...
        default: {
            ...
            printf("DAAL FATAL ERROR: Cannot load neither libdaal_thread.dll nor libdaal_sequential.dll.\n");
            exit(1);
        }
    }
}
...
FARPROC load_daal_thr_func(char *ordinal)
{
    ...
    {
        printf("DAAL FATAL ERROR: Cannot load \"%s\" function because threaded layer DLL isn`t loaded.\n", ordinal);
        exit(1);
    }
    ...
    if(FuncAddress == NULL)
    {
        printf("DAAL FATAL ERROR: Cannot load \"%s\" function.\n", ordinal);
        exit(1);
    }
...
CALL_RET_FUNC_FROM_DLL(dnnError_t, fpk_dnn_, ConvolutionCreateForwardBias_F32, (        \
        ...
        pConvolution, algorithm, dimension,                                             \
