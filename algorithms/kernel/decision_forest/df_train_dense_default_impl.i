...
{
    return (val > algorithmFPType(10)*data_management::data_feature_utils::getEpsilonVal<algorithmFPType>());
}
    ...
    const Parameter& par, size_t nClasses)
{
    ...
    DAAL_CHECK(!res.varImp || varImpBD.get(), ErrorMemoryAllocationFailed);
    ...
    if(par.resultsToCompute & computeOutOfBagError)
    {
        ...
        *oobErr.get() = mainCtx.finalizeOOBError(y);
    }
...
protected:
    ...
    TrainBatchTaskBase(size_t seed, const NumericTable *x, const NumericTable *y, const Parameter& par,
        ...
        _accuracy(data_management::data_feature_utils::getEpsilonVal<algorithmFPType>())
    {
    ...
    const typename DataHelper::TSplitData& split)
{
    ...
    if((val < data_management::data_feature_utils::getEpsilonVal<algorithmFPType>()) &&
        (val > -data_management::data_feature_utils::getEpsilonVal<algorithmFPType>()))
        val = 0;
