...
{
public:
    typedef decision_forest::internal::TreeImpClassification<algorithmFPType> TreeType;
    ...
    const decision_forest::classification::Model *m, NumericTable *r, size_t nClasses)
{
    ...
    if(!task.run(nClasses))
        return services::Status(services::ErrorMemoryAllocationFailed);
    return services::Status();
}

template <typename algorithmFPType, CpuType cpu>
bool PredictClassificationTask<algorithmFPType, cpu>::run(size_t nClasses)
{
