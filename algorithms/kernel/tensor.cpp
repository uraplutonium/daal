...
void HomogenTensor<DataType>::getTSubtensor( size_t fixedDims, const size_t *fixedDimNums, size_t rangeDimIdx, size_t rangeDimNum,
                                             int rwFlag, SubtensorDescriptor<T> &block, TensorIface::DataLayout layout )
{
    ...
    size_t blockSize = block.setDetails( nDim, dimSizes, fixedDims, fixedDimNums, rangeDimIdx, rangeDimNum, rwFlag, layout );
...
void HomogenTensor<DataType>::releaseTSubtensor( SubtensorDescriptor<T> &block )
{
    ...
    {
        if( _layout.isLayout( block.getLayout() ) )
        {
            if( !IsSameType<T, DataType>::value )
            {
                const size_t *_dimOffsets = &((_layout.getOffsets())[0]);
...
template<>                                                                                                                 \
void HomogenTensor<T1>::getSubtensor(size_t fixedDims, const size_t *fixedDimNums, size_t rangeDimIdx, size_t rangeDimNum, \
                  ReadWriteMode rwflag, SubtensorDescriptor<T2> &block, TensorIface::DataLayout layout )                   \
{                                                                                                                          \
...
template void HomogenTensor<T1>::getTSubtensor( size_t, const size_t *, size_t, size_t, int, SubtensorDescriptor<T2> &, TensorIface::DataLayout );  \
