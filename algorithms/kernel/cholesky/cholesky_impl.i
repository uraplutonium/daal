...
void CholeskyKernel<algorithmFPType, method, cpu>::compute(const size_t na, const NumericTable *const *a,
                                                           const size_t nr, NumericTable *r[], const daal::algorithms::Parameter *par)
{
    NumericTable *aTable = const_cast<NumericTable *>( a[0] );     /* Input data */
    ...
    {
        rMicroTable = new BlockMicroTable<algorithmFPType, writeOnly, cpu>(r[0]);
        rMicroTable->getBlockOfRows(0, dim, &L);
    }
    else
    {
        rPackedMicroTable = new PackedArrayMicroTable<algorithmFPType, writeOnly, cpu>(r[0]);
                                                                   ...
                                                                   algorithmFPType **pL, MKL_INT dim)
{
    ...
    if(info > 0)
    {
        ...
        error->setId(services::ErrorInputMatrixHasNonPositiveMinor);
        error->addIntDetail(services::Minor, (int)info);
