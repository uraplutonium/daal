...
services::Status ImplicitALSTrainDistrStep1Kernel<algorithmFPType, cpu>::compute(
                 ...
                 const Parameter *parameter)
{
    ...
    DAAL_KERNEL_CHECK_EX(nRowsRead >= nFactors, services::ErrorIncorrectNumberOfRowsInInputNumericTable);
    ...
    for (size_t block = 0; block < nBlocks; block++)
    {
        ...
        DAAL_KERNEL_CHECK_EX(nRowsRead >= nRowsToCP, services::ErrorIncorrectNumberOfRowsInInputNumericTable);
...
template <typename algorithmFPType, CpuType cpu>
services::Status ImplicitALSTrainDistrStep2Kernel<algorithmFPType, cpu>::compute(
...
template <typename algorithmFPType, CpuType cpu>
services::Status ImplicitALSTrainDistrStep3Kernel<algorithmFPType, cpu>::compute(
                 ...
                 data_management::KeyValueDataCollection *dstPartialModels, const Parameter *parameter)
{
    ...
    DAAL_KERNEL_CHECK_EX(nRowsRead >= 1, services::ErrorIncorrectNumberOfRowsInInputNumericTable);
    ...
    daal::threader_for(nBlocks, nBlocks, [ & ](size_t i)
    {
        ...
        // DAAL_KERNEL_CHECK_EX(nRowsReadLocal >= nRows, services::ErrorIncorrectNumberOfRowsInInputNumericTable); // safeStatus is needed

        for (size_t j = 0; j < nRows; j++)
        {
            algorithmFPType *srcFactor;
            nRowsReadLocal = mtSrcFactors.getBlockOfRows(indices[j] - offset, 1, &srcFactor);
            // DAAL_KERNEL_CHECK_EX(nRowsReadLocal >= 1, services::ErrorIncorrectNumberOfRowsInInputNumericTable);
...
template <typename algorithmFPType, CpuType cpu>
services::Status ImplicitALSTrainDistrStep4Kernel<algorithmFPType, fastCSR, cpu>::compute(
                 ...
                 const Parameter *parameter)
{
    ...
    DAAL_KERNEL_CHECK_EX(nFactorsRows || indices || mtIndices, services::ErrorMemoryAllocationFailed);
    ...
    for (size_t i = 0; i < nBlocks; i++)
    {
        ...
        DAAL_KERNEL_CHECK_EX(nRowsRead >= nRows, services::ErrorIncorrectNumberOfRowsInInputNumericTable);
    }
