...
{
    __DAAL_INTIALIZE_KERNELS(internal::ImplicitALSPredictKernel, algorithmFPType);
}
...
void DistributedContainer<step1Local, algorithmFPType, method, cpu>::compute()
{
    ...
    services::SharedPtr<NumericTable> usersFactorsTable = usersFactors->getFactors();
    services::SharedPtr<NumericTable> itemsFactorsTable = itemsFactors->getFactors();
