...
services::Status DistributedInput<step1Local>::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    services::Status s;
    s |= checkNumericTable(usersModel->getFactors().get(), usersFactorsStr(), unexpectedLayouts, 0, nFactors);
    if(!s) return s;
    size_t nRowsUsersModel = usersModel->getFactors()->getNumberOfRows();
    s |= checkNumericTable(usersModel->getIndices().get(), usersIndicesStr(), unexpectedLayoutsIndices, 0, 1, nRowsUsersModel);
    if(!s) return s;

    s |= checkNumericTable(itemsModel->getFactors().get(), itemsFactorsStr(), unexpectedLayouts, 0, nFactors);
    if(!s) return s;
    size_t nRowsItemsModel = itemsModel->getFactors()->getNumberOfRows();
    s |= checkNumericTable(itemsModel->getIndices().get(), itemsIndicesStr(), unexpectedLayoutsIndices, 0, 1, nRowsItemsModel);
    return s;
}
