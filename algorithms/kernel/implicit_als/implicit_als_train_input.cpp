...
services::Status Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(method == defaultDense)
    {
        ...
        s |= checkNumericTable(get(data).get(), dataStr(), unexpectedLayouts);
        if(!s) return s;
    }
    else
    {
    ...
    s |= checkNumericTable(model->getUsersFactors().get(), usersFactorsStr(), unexpectedLayouts, 0, nFactors, nUsers);
    if(!s) return s;
    s |= checkNumericTable(model->getItemsFactors().get(), itemsFactorsStr(), unexpectedLayouts, 0, nFactors, nItems);
    return s;
}
