    ...
    algorithmFPType **alphaPtr)
{
    ...
    for (size_t m = 0; m < maxIter; m++)
    {
        ...
        services::Status s = learnerTrain->computeNoThrow();
        if(!s) {daal::services::daal_free(errFlag); this->_errors->add(s.getCollection()->getErrors()); return;}
                                                                            ...
                                                                            Model *r, const Parameter *par)
{
    ...
    DAAL_KERNEL_CHECK(this->_errors->isEmpty());
