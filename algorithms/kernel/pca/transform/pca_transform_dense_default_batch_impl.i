    ...
    daal::threader_for( numBlocks, numBlocks, [ =, &tls, &transformedData, &data, &safeStat ](int iBlock)
    {
        ...
        if (isNormalize)
        {
            ...
            for (size_t rowId = 0; rowId < numRows; ++rowId)
            {
                ...
                {
                    pCopyBlock[rowId * numInvSigmas + colId] = pDataBlock[rowId * numInvSigmas + colId] * pInvSigmas[colId];
                }
            }
        ...
        /* compute whitening to unit variance of transformed data if numInvEigenvalues!= 0*/
        for (size_t rowId = 0; rowId < numInvEigenvalues; ++rowId)
        {
            PRAGMA_IVDEP
            PRAGMA_VECTOR_ALWAYS
