...
void PCASVDOnlineKernel<interm, cpu>::compute(const services::SharedPtr<data_management::NumericTable> &data,
...
template <typename interm, CpuType cpu>
void PCASVDOnlineKernel<interm, cpu>::finalizeMerge(const services::SharedPtr<data_management::NumericTable> &nObservationsTable,
                            ...
                            const interm *sums, const interm *ssq, const interm *mean, interm *variance)
{
    for (size_t j = 0; j < _nFeatures; j++)
    {
        variance[j] =  sSqrt<cpu>((ssq[j] - 2 * mean[j] * sums[j] + nObservations * mean[j] * mean[j]) / (nObservations - 1));
    }
}
