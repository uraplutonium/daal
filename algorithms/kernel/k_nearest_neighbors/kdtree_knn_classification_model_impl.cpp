...
__DAAL_REGISTER_SERIALIZATION_CLASS(PartialModel, SERIALIZATION_K_NEAREST_NEIGHBOR_PARTIALMODEL_ID);
...
PartialModel::PartialModel(size_t nFeatures) : _impl(new PartialModelImpl(nFeatures)) {}

PartialModel::PartialModel(size_t nFeatures, services::Status &st) : _impl(new PartialModelImpl(nFeatures))
{
   if(!_impl) { st.add(services::ErrorMemoryAllocationFailed); }
}

PartialModelPtr PartialModel::create(size_t nFeatures, services::Status *stat)
{
    DAAL_DEFAULT_CREATE_IMPL_EX(PartialModel, nFeatures);
}
...
services::Status PartialModel::serializeImpl(data_management::InputDataArchive  * arch)
{
...
services::Status PartialModel::deserializeImpl(const data_management::OutputDataArchive * arch)
{
    daal::algorithms::Model::serialImpl<const data_management::OutputDataArchive, true>(arch);
    _impl->serialImpl<const data_management::OutputDataArchive, true>(arch);

    return services::Status();
}

size_t PartialModel::getNumberOfFeatures() const
{
    return _impl->getNumberOfFeatures();
}
