...
class PartitioningKDTreeTable : public data_management::AOSNumericTable
{
public:
    PartitioningKDTreeTable(size_t rowCount = 0) : data_management::AOSNumericTable(sizeof(PartitioningKDTreeNode), 5, rowCount)
    {
        setFeature<size_t> (0, DAAL_STRUCT_MEMBER_OFFSET(PartitioningKDTreeNode, dimension));
        setFeature<size_t> (1, DAAL_STRUCT_MEMBER_OFFSET(PartitioningKDTreeNode, leftIndex));
        setFeature<size_t> (2, DAAL_STRUCT_MEMBER_OFFSET(PartitioningKDTreeNode, rightIndex));
        setFeature<double> (3, DAAL_STRUCT_MEMBER_OFFSET(PartitioningKDTreeNode, cutPoint));
        setFeature<bool>   (4, DAAL_STRUCT_MEMBER_OFFSET(PartitioningKDTreeNode, isLeaf));
        allocateDataMemory();
    }

    PartitioningKDTreeTable(const PartitioningKDTreeTable &) = delete;
};

typedef services::SharedPtr<PartitioningKDTreeTable> PartitioningKDTreeTablePtr;
typedef services::SharedPtr<const PartitioningKDTreeTable> PartitioningKDTreeTableConstPtr;
...
public:
    ...
    */
    ...
    DAAL_EXPORT void setData(const data_management::NumericTablePtr & value, bool copy)
    {
        ...
        {
            SOANumericTablePtr tbl = SOANumericTable::create(value->getNumberOfColumns(),
                                                             value->getNumberOfRows(),
                                                             DictionaryIface::equal);
            ...
            BlockDescriptor<algorithmFPType> destBD, srcBD;
            tbl->getBlockOfRows(0, tbl->getNumberOfRows(), writeOnly, destBD);
            value->getBlockOfRows(0, value->getNumberOfRows(), readOnly, srcBD);
...
class PartialModel::PartialModelImpl : private kdtree_knn_classification::interface1::Model::ModelImpl
{
private:
    typedef kdtree_knn_classification::interface1::Model::ModelImpl BaseType;

public:
    /**
     * Empty constructor for deserialization
     */
    PartialModelImpl(size_t nFeatures) : BaseType(nFeatures), _partitioningKDTreeTable(), _partitioningLastNodeIndex(0) {}

    using BaseType::getKDTreeTable;
    using BaseType::setKDTreeTable;
    using BaseType::getRootNodeIndex;
    using BaseType::setRootNodeIndex;
    using BaseType::getLastNodeIndex;
    using BaseType::setLastNodeIndex;
    using BaseType::getData;
    using BaseType::setData;
    using BaseType::getLabels;
    using BaseType::setLabels;
    using BaseType::getNumberOfFeatures;

    /**
     * Returns the partitioning KD-tree table
     * \return KD-tree table
     */
    PartitioningKDTreeTablePtr getPartitioningKDTreeTable() { return _partitioningKDTreeTable; }

    /**
     * Returns the partitioning KD-tree table
     * \return KD-tree table
     */
    PartitioningKDTreeTableConstPtr getPartitioningKDTreeTable() const { return _partitioningKDTreeTable; }

    /**
    *  Sets a partitioning KD-tree table
    *  \param[in]  value  KD-tree table
    */
    void setPartitioningKDTreeTable(const services::SharedPtr<PartitioningKDTreeTable> & value) { _partitioningKDTreeTable = value; }

    /**
     * Returns the index of first part KD-tree last node
     * \return Index of first part KD-tree last node
     */
    size_t getPartitioningLastNodeIndex() const { return _partitioningLastNodeIndex; }

    /**
    *  Sets a index of first part KD-tree last node
    *  \param[in]  value  Index of first part KD-tree last node
    */
    void setPartitioningLastNodeIndex(size_t value) { _partitioningLastNodeIndex = value; }

    template<typename Archive, bool onDeserialize>
    void serialImpl(Archive * arch)
    {
        BaseType::serialImpl<Archive, onDeserialize>(arch);
        arch->set(_partitioningLastNodeIndex);
        arch->setSharedPtrObj(_partitioningKDTreeTable);
    }

private:
    PartitioningKDTreeTablePtr _partitioningKDTreeTable;
    size_t _partitioningLastNodeIndex;
};
...
using interface1::PartitioningKDTreeTable;
using interface1::PartitioningKDTreeTablePtr;
using interface1::PartitioningKDTreeTableConstPtr;
using interface1::PartitioningKDTreeNode;
