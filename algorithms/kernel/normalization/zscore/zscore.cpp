...
{
    services::Status s;
    DAAL_CHECK_STATUS(s, data_management::checkNumericTable(get(data).get(),dataStr()));
    if (method == sumDense)
    {
        size_t nFeatures = get(data)->getNumberOfColumns();
        DAAL_CHECK_STATUS(s, data_management::checkNumericTable(get(data)->basicStatistics.get(data_management::NumericTableIface::sum).get(),
 ...
 * \param[in] method Algorithm computation method
 */
services::Status Result::check(const daal::algorithms::Input *in, const daal::algorithms::Parameter *par, int method) const
{
    ...
    return data_management::checkNumericTable(get(normalizedData).get(), normalizedDataStr(), unexpectedLayouts, 0, nFeatures, nVectors);
}
