...
DAAL_EXPORT ParameterBase::ParameterBase(double lowerBound, double upperBound,
    const services::SharedPtr<low_order_moments::BatchImpl> &moments) :
    lowerBound(lowerBound), upperBound(upperBound), moments(moments) { }

/**
 * Check the correctness of the %ParameterBase object
 */
DAAL_EXPORT services::Status ParameterBase::check() const
{
    if (!moments) return services::Status(services::ErrorNullParameterNotSupported);
    if (lowerBound >= upperBound) return services::Status(services::ErrorLowerBoundGreaterThanOrEqualToUpperBound);
