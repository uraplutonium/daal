...
void BrownBoostTrainKernel<method, algorithmFPType, cpu>::compute(size_t na, services::SharedPtr<NumericTable> *a,
                                                                  Model *r, const Parameter *par)
{
    services::SharedPtr<NumericTable> xTable = a[0];
    services::SharedPtr<NumericTable> yTable = a[1];
    ...
    services::SharedPtr<NumericTable> weakLearnerInputTables[] = {xTable, yTable, wTable};
    ...
    services::SharedPtr<NumericTable> alphaTable = r->getAlpha();
...
void BrownBoostTrainKernel<method, algorithmFPType, cpu>::brownBoostFreundKernel(
    size_t nVectors, services::SharedPtr<NumericTable> weakLearnerInputTables[],
    ...
    algorithmFPType **alphaPtr)
{
    ...
    learnerPredict->input.set(classifier::prediction::data, weakLearnerInputTables[0]);
    ...
    for (size_t iteration = 0; iteration < parameter->maxIterations && s > zero; iteration++)
    {
        ...
        learnerPredict->input.set(classifier::prediction::model, learnerModel);
        learnerPredict->compute();
            ...
            const algorithmFPType *r, algorithmFPType *nra, algorithmFPType *nre2, algorithmFPType *w)
{
    ...
    vExp<cpu>(nVectors, w, w);
    vErf<cpu>(nVectors, nre2, nre2);
            ...
            size_t nVectors, Parameter *parameter, services::SharedPtr<services::KernelErrorCollection> _errors)
{
    ...
    sqrtC = sErfInv<cpu>(one - error);
    c     = sqrtC * sqrtC;
    invC  = one / c;
    invSqrtC = one / sqrtC;
    sqrtPiC = sSqrt<cpu>(pi * c);
}
            ...
            algorithmFPType *h, algorithmFPType *y)
{
    ...
    for (size_t nrIter = 0; !nrDone && nrIter < nrMaxIter; nrIter++)
    {
        ...
        vExp<cpu>(nVectors, nrw,  nrw);
        vErf<cpu>(nVectors, nre1, nre1);
        for (size_t j = 0; j < nVectors; j++)
        {
        ...
        if (sFabs<algorithmFPType, cpu>(nrB / nrW) <= nu) { nrDone = true; }
        if (sFabs<algorithmFPType, cpu>(nrB) <= nrAccuracy &&
            sFabs<algorithmFPType, cpu>(nrE) <= nrAccuracy) { nrDone = true; }
    }
