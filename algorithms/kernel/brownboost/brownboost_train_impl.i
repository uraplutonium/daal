                                                                              ...
                                                                              Model *r, const Parameter *par)
{
    ...
    alphaTable->resize(nWeakLearners);
    ...
    algorithmFPType **alphaPtr)
{
    ...
    for (size_t iteration = 0; iteration < parameter->maxIterations && s > zero; iteration++)
    {
        ...
        services::Status status = learnerTrain->computeNoThrow();
        if(!status) {daal::services::daal_free(r); this->_errors->add(status.getCollection()->getErrors()); return;}
        ...
        status |= learnerPredict->computeNoThrow();
        if(!status) {daal::services::daal_free(r); this->_errors->add(status.getCollection()->getErrors()); return;}
...
template <Method method, typename algorithmFPType, CpuType cpu>
services::Status NewtonRaphsonKernel<method, algorithmFPType, cpu>::compute(algorithmFPType gamma, algorithmFPType s,
