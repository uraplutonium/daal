...
void BrownBoostPredictKernel<method, algorithmFPType, cpu>::compute(services::SharedPtr<NumericTable> xTable,
                                                                    const Model *m, services::SharedPtr<NumericTable> rTable,
                                                                    const Parameter *par)
{
    ...
    services::SharedPtr<NumericTable> alphaTable = boostModel->getAlpha();
    ...
    if (error != zero)
    {
        algorithmFPType sqrtC = sErfInv<cpu>(one - error);
        invSqrtC = one / sqrtC;
