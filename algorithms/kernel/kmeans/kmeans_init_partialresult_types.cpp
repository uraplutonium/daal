...
services::Status PartialResult::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    {
        s |= checkNumericTable(pPartialClusters.get(), partialClustersStr(), unexpectedLayouts, 0, inputFeatures, nClusters);
        if(!s) return s;
    }
    s |= checkNumericTable(get(partialClustersNumber).get(), partialClustersNumberStr(), unexpectedLayouts, 0, 1, 1);
    if(!s) return s;
...
services::Status DistributedStep2LocalPlusPlusPartialResult::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    services::Status s;
    s |= checkNumericTable(get(outputOfStep2ForStep3).get(), outputOfStep2ForStep3Str(), (int)packed_mask, 0, 1, 1);
    if(!s) return s;
...
services::Status DistributedStep4LocalPlusPlusPartialResult::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    services::Status s;
    s |= checkNumericTable(get(outputOfStep4).get(), outputOfStep4Str(), (int)packed_mask, 0, nFeatures, nRows);
    return s;
}
...
services::Status DistributedStep5MasterPlusPlusPartialResult::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    services::Status s;
    s |= checkNumericTable(get(candidates).get(), candidatesStr(), (int)packed_mask, 0, nFeatures, nMaxCandidates);
    if(!s) return s;
...
services::Status DistributedStep5MasterPlusPlusPartialResult::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    const size_t nMaxCandidates = size_t(stepPar->oversamplingFactor*stepPar->nClusters)*stepPar->nRounds + 1;
    services::Status s;
    s |= checkNumericTable(get(candidates).get(), candidatesStr(), (int)packed_mask, 0, 0, nMaxCandidates);
    if(!s) return s;
