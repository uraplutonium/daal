...
services::Status KMeansBatchKernel<method, algorithmFPType, cpu>::compute( size_t na, const NumericTable *const *a,
                                                                           size_t nr, const NumericTable *const *r, const Parameter *par)
{
    ...
    for(kIter = 0; kIter < nIter; kIter++)
    {
        ...
        if(!task) { daal::services::daal_free( clusterS0 ); daal::services::daal_free( clusterS1 ); DAAL_RETURN_STATUS() }
