...
services::Status DistributedStep2MasterInput::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    services::Status s;
    s |= checkNumericTable(firstPres->get(nObservations).get(), nObservationsStr(),
        unexpectedLayouts, 0, 1, kmPar->nClusters);
    if(!s) return s;
    s |= checkNumericTable(firstPres->get(partialSums).get(), partialSumsStr(),
        unexpectedLayouts, 0, 0, kmPar->nClusters);
    if(!s) return s;
    s |= checkNumericTable(firstPres->get(partialObjectiveFunction).get(), partialGoalFunctionStr(),
        unexpectedLayouts, 0, 1, 1);
    if(!s) return s;
    if( kmPar->assignFlag )
    {
        s |= checkNumericTable(firstPres->get(partialAssignments).get(), partialAssignmentsStr(),
    ...
    for(size_t i = 1; i < nBlocks; i++)
    {
        ...
        s |= checkNumericTable(pres->get(nObservations).get(), nObservationsStr(),
            unexpectedLayouts, 0, 1, kmPar->nClusters);
        if(!s) return s;
        s |= checkNumericTable(pres->get(partialSums).get(), partialSumsStr(),
            unexpectedLayouts, 0, inputFeatures, kmPar->nClusters);
        if(!s) return s;
        s |= checkNumericTable(pres->get(partialObjectiveFunction).get(), partialGoalFunctionStr(),
            unexpectedLayouts, 0, 1, 1);
        if(!s) return s;
        if( kmPar->assignFlag )
        {
            s |= checkNumericTable(pres->get(partialAssignments).get(), partialAssignmentsStr(),
                unexpectedLayouts, 0, 1);
            if(!s) return s;
        }
    }
