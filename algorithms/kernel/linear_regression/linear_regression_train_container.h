...
{
    __DAAL_INTIALIZE_KERNELS(internal::LinearRegressionTrainBatchKernel, interm, method);
}
...
void BatchContainer<interm, method, cpu>::compute()
{
    ...
    SharedPtr<NumericTable> x = input->get(data);
    SharedPtr<NumericTable> y = input->get(dependentVariables);

    SharedPtr<linear_regression::Model> r = result->get(model);
...
{
    __DAAL_INTIALIZE_KERNELS(internal::LinearRegressionTrainOnlineKernel, interm, method);
}
...
void OnlineContainer<interm, method, cpu>::compute()
{
    ...
    SharedPtr<linear_regression::Model> partialModel = partialResult->get(training::partialModel);
...
void OnlineContainer<interm, method, cpu>::finalizeCompute()
{
    ...
    SharedPtr<linear_regression::Model> partialModel = partialResult->get(training::partialModel);
    SharedPtr<linear_regression::Model> model = result->get(training::model);
 ...
 */
template <typename interm, training::Method method, CpuType cpu>
DistributedContainer<step1Local, interm, method, cpu>::DistributedContainer(daal::services::Environment::env *daalEnv)
{
    __DAAL_INTIALIZE_KERNELS(internal::LinearRegressionTrainOnlineKernel, interm, method);
}

template <typename interm, training::Method method, CpuType cpu>
DistributedContainer<step1Local, interm, method, cpu>::~DistributedContainer()
{
    __DAAL_DEINTIALIZE_KERNELS();
}
 ...
 */
template <typename interm, training::Method method, CpuType cpu>
void DistributedContainer<step1Local, interm, method, cpu>::compute()
{
    Input *input = static_cast<Input *>(_in);
    PartialResult *partialResult = static_cast<PartialResult *>(_pres);
 ...
 */
template <typename interm, training::Method method, CpuType cpu>
void DistributedContainer<step1Local, interm, method, cpu>::finalizeCompute()
{
    PartialResult *partialResult = static_cast<PartialResult *>(_pres);
    Result *result = static_cast<Result *>(_res);
...
{
    __DAAL_INTIALIZE_KERNELS(internal::LinearRegressionTrainDistributedKernel, interm, method);
}
...
void DistributedContainer<step2Master, interm, method, cpu>::compute()
{
    ...
    services::SharedPtr<data_management::DataCollection> collection
        = static_cast<services::SharedPtr<data_management::DataCollection> >(input->get(partialModels));
