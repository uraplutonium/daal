 ...
 */
daal::services::Status daal::algorithms::linear_regression::checkModel(
    linear_regression::Model* model, const daal::algorithms::Parameter *par, const size_t coefdim, const size_t nrhs, int method)
{
    using namespace daal::services;
    ...
    services::Status s;
    DAAL_CHECK_STATUS(s, data_management::checkNumericTable(model->getBeta().get(), betaStr(), 0, 0, coefdim, nrhs));
    ...
    if(method == linear_regression::training::normEqDense)
    {
        ...
        DAAL_CHECK_STATUS(
            s, data_management::checkNumericTable(modelNormEq->getXTXTable().get(), XTXTableStr(), 0, 0, dimWithoutBeta, dimWithoutBeta));

        DAAL_CHECK_STATUS(
            s, data_management::checkNumericTable(modelNormEq->getXTYTable().get(), XTYTableStr(), 0, 0, dimWithoutBeta, nrhs));
    }
    else if(method == linear_regression::training::qrDense)
    {
        ...
        DAAL_CHECK_STATUS(
            s, data_management::checkNumericTable(modelQR->getRTable().get(), RTableStr(), 0, 0, dimWithoutBeta, dimWithoutBeta));

        DAAL_CHECK_STATUS(
            s, data_management::checkNumericTable(modelQR->getQTYTable().get(), QTYTableStr(), 0, 0, dimWithoutBeta, nrhs));
    }
