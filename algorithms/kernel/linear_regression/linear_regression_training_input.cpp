...
{
    DAAL_CHECK(Argument::size() == 2,  Error::create(ErrorIncorrectNumberOfInputNumericTables));
    ...
    DAAL_CHECK(nRowsInData >= nColumnsInData, Error::create(ErrorIncorrectNumberOfObservations));

    return data_management::checkNumericTable(dependentVariableTable.get(), dependentVariableStr(), 0, 0, 0, nRowsInData);
}
