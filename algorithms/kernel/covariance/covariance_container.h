...
#define __DAAL_COVARIANCE_BATCH_CONTAINER_CONSTRUCTOR(ComputeMethod, KernelClass)   \
    ...
    {                                                                               \
        __DAAL_INTIALIZE_KERNELS(KernelClass, algorithmFPType, ComputeMethod);      \
    }
...
#define __DAAL_COVARIANCE_BATCH_CONTAINER_COMPUTE(ComputeMethod, KernelClass)       \
    ...
    {                                                                               \
        ...
        SharedPtr<NumericTable> covTable  = result->get(covariance);                \
...
#define __DAAL_COVARIANCE_ONLINE_CONTAINER_COMPUTE(ComputeMethod, KernelClass)          \
    ...
    {                                                                                   \
        ...
        SharedPtr<NumericTable> crossProductTable = partialResult->get(crossProduct);   \
        SharedPtr<NumericTable> sumTable          = partialResult->get(sum);            \
                                                                                        \
        Parameter *parameter = static_cast<Parameter *>(_par);                          \
        daal::services::Environment::env &env = *_env;                                  \
                                                                                        \
...
{
    __DAAL_INTIALIZE_KERNELS(internal::CovarianceDistributedKernel, algorithmFPType, method);
}
...
void DistributedContainer<step2Master, algorithmFPType, method, cpu>::compute()
{
    ...
    SharedPtr<DataCollection> collection = input->get(partialResults);
