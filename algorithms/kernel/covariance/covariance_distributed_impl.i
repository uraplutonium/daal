...
void CovarianceDistributedKernel<algorithmFPType, method, cpu>::compute(
            SharedPtr<DataCollection> &partialResultsCollection,
            SharedPtr<NumericTable> &nObservationsTable, SharedPtr<NumericTable> &crossProductTable,
            SharedPtr<NumericTable> &sumTable, const Parameter *parameter)
{
    ...
    *nObservations = zero;
    ...
    SharedPtr<NumericTable> partialCrossProductTable, partialSumsTable, partialNObservationsTable;
    ...
    {
        services::SharedPtr<PartialResult> patrialResult =
            services::staticPointerCast<PartialResult, SerializationIface>((*partialResultsCollection)[i]);
        partialCrossProductTable  = patrialResult->get(covariance::crossProduct);
        partialSumsTable          = patrialResult->get(covariance::sum);
        partialNObservationsTable = patrialResult->get(covariance::nObservations);
            ...
            SharedPtr<NumericTable> &meanTable, const Parameter *parameter)
{
    finalizeCovariance<algorithmFPType, cpu>(crossProductTable, sumTable, nObservationsTable,
        covTable, meanTable, parameter, this->_errors);
}
