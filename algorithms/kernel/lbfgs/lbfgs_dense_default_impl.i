...
void LBFGSKernel<algorithmFPType, defaultDense, cpu>::compute(
            Input *input, Result *result, Parameter *parameter)
{
    size_t argumentSize = input->get(inputArgument)->getNumberOfColumns();
    ...
    if (nIterations == 0)
    {
        ...
        LBFGSTask<algorithmFPType, cpu> task(argumentSize,
            result->get(minimum).get(), input->get(inputArgument).get(), _errors);
        setNIterations(result->get(lbfgs::nIterations).get(), 0);
        return;
    }
    ...
    LBFGSTask<algorithmFPType, cpu> task(argumentSize, nTerms, m, batchSize, parameter->batchIndices.get(),
        ...
        result->get(minimum).get(), input->get(inputArgument).get(),
    ...
    for (; t < nLIterations;)
    {
        for (; epoch < (t + 1) * L; epoch++)
        {
            ...
            if (!updateArgument(t, epoch, m, correctionIndex, nTerms, argumentSize, batchSize, nStepLength,
                                ...
                                argument))
            { setNIterations(result->get(lbfgs::nIterations).get(), epoch); return; }
        }
        ...
        if (t >= 2)
        {
            ...
            if (hessianFunction->getErrors()->size() != 0) { setNIterations(result->get(lbfgs::nIterations).get(), epoch); return; }
    ...
    setNIterations(result->get(lbfgs::nIterations).get(), epoch);
}
 ...
 *                             On output: Result of two-loop recursion.
