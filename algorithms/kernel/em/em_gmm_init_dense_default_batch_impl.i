...
void EMInitKernel<AlgorithmFPType, method, cpu>::compute(const SharedPtr<NumericTable>& data, const SharedPtr<NumericTable>& weightsToInit,
        const SharedPtr<NumericTable>& meansToInit, const SharedPtr<DataCollection>& covariancesToInit, Parameter *parameter)
{
    ...
    sigma = SharedPtr<DataCollection>(new DataCollection());
...
template<typename AlgorithmFPType, Method method, CpuType cpu>
void EMInitKernel<AlgorithmFPType, method, cpu>::writeValuesToTables(const SharedPtr<NumericTable>& weightsToInit,
        const SharedPtr<NumericTable>& meansToInit, const SharedPtr<DataCollection>& covariancesToInit)
{
    ...
    {
        SharedPtr<NumericTable> covariance = staticPointerCast<NumericTable, SerializationIface>((*covariancesToInit)[k]);
...
ErrorID EMInitKernel<AlgorithmFPType, method, cpu>::runEM()
{
    ...
    em.input.set(daal::algorithms::em_gmm::data, data);
    em.input.set(daal::algorithms::em_gmm::inputWeights, alpha);
    em.input.set(daal::algorithms::em_gmm::inputMeans, means);
    em.input.set(daal::algorithms::em_gmm::inputCovariances, sigma);

    SharedPtr<daal::algorithms::em_gmm::Result> emResult(new daal::algorithms::em_gmm::Result());
    emResult->set(daal::algorithms::em_gmm::weights, alpha);
    emResult->set(daal::algorithms::em_gmm::means, means);
    emResult->set(daal::algorithms::em_gmm::covariances, sigma);

    SharedPtr<HomogenNumericTableCPU<AlgorithmFPType, cpu> > loglikelyhoodValueTable(new HomogenNumericTableCPU<AlgorithmFPType, cpu>(1, 1));

    SharedPtr<HomogenNumericTableCPU<int, cpu> > nIterationsValueTable(new HomogenNumericTableCPU<int, cpu>(1, 1));
    ...
    em.setResult(emResult);
    ErrorID returnErrorId = (ErrorID) 0;
    em.compute();
    if(em.getErrors()->size() != 0)
    {
        loglikelyhood = -MaxVal<AlgorithmFPType, cpu>::get();
        returnErrorId = ErrorEMInitNoTrialConverges;
    }
    else
    {
