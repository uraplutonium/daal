...
{
    __DAAL_INTIALIZE_KERNELS(internal::EMKernel, algorithmFPType, method);
}
...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    services::SharedPtr<data_management::DataCollection> initCovs = input->get(inputCovariances);
    ...
    services::SharedPtr<data_management::DataCollection> resultCovs = pRes->get(covariances);
    for(size_t i = 0; i < nComponents; i++)
    {
