...
public:
    ...
    void compute(const SharedPtr<NumericTable>& data, const SharedPtr<NumericTable>& weightsToInit, const SharedPtr<NumericTable>& meansToInit,
                 const SharedPtr<DataCollection>& covariancesToInit, Parameter *par);

private:
    void writeValuesToTables(const SharedPtr<NumericTable>& weightsToInit, const SharedPtr<NumericTable>& meansToInit,
                             const SharedPtr<DataCollection>& covariancesToInit);
    void setSelectedSetAsInitialValues(int *selectedSet);
    ErrorID runEM();
    void generateSelectedSet(int *selectedSet, size_t length, int seed);

    SharedPtr<NumericTable> data;
    size_t nComponents;
    size_t nFeatures;
    size_t nVectors;
    size_t nTrials;
    ...
    SharedPtr<DataCollection> sigma;
