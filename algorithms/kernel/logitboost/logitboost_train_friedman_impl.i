...
void LogitBoostTrainKernel<friedman, algorithmFPType, cpu>::compute(const size_t na, services::SharedPtr<NumericTable> a[],
            Model *r, const Parameter *par)
{
    ...
    services::SharedPtr<NumericTable> x = a[0];
    services::SharedPtr<NumericTable> y = a[1];
    ...
    learnerPredict->input.set(classifier::prediction::data, x);
