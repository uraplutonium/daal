...
Status Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    s |= checkNumericTable(predictedLabelsTable.get(), predictedLabelsStr(), unexpectedLayouts, 0, 1);
    if(!s) return s;

    size_t nRows = predictedLabelsTable->getNumberOfRows();
    s |= checkNumericTable(groundTruthLabelsTable.get(), groundTruthLabelsStr(), unexpectedLayouts, 0, 1, nRows);
    return s;
}
...
Status Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    s |= checkNumericTable(confusionMatrixTable.get(), confusionMatrixStr(), unexpectedLayouts, 0, nClasses, nClasses);
    if(!s) return s;
    s |= checkNumericTable(multiClassMetricsTable.get(), multiClassMetricsStr(), unexpectedLayouts, 0, 8, 1);
    return s;
}
