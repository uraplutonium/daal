...
services::Status Input::checkImpl(const daal::algorithms::Parameter *parameter) const
{
    ...
    if(parameter != NULL)
    {
        ...
        if (algParameter->nClasses < 2) return services::Status(services::ErrorIncorrectNumberOfClasses);
    }
    ...
    if(!m) { s.add(services::ErrorNullModel); return s; }
    ...
    {
        return services::Status(services::Error::create(services::ErrorIncorrectNumberOfColumns, services::ArgumentName, dataStr()));
    }
    return s;
}
