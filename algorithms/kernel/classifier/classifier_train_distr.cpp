...
services::Status PartialResult::checkImpl(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter) const
{
    ...
    if(!m) return services::Status(services::ErrorNullModel);
...
services::Status DistributedInput::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if (models == 0) { return services::Status(services::ErrorNullModel); }

    size_t size = models->size();
    if (size == 0) { return services::Status(services::ErrorIncorrectNumberOfElementsInInputCollection); }

    for (size_t i = 0; i < size; i++)
    {
        classifier::Model *model = (classifier::Model *)((*models)[i].get());
        if (model == 0) { return services::Status(services::ErrorNullModel); }
    }
    return services::Status();
}
