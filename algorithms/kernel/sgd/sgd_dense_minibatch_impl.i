...
void SGDKernel<algorithmFPType, miniBatch, cpu>::compute(Input *input, Result *result, Parameter<miniBatch> *parameter)
{
    ...
    if(maxIterations == 0 || L == 0)
    {
        SGDMiniBatchTask<algorithmFPType, cpu> task(
            ...
            nFeatures,
            result->get(minimum),
            input->get(inputArgument),
            result->get(nIterations)
        );
