...
services::Status SVMTrainImpl<boser, algorithmFPType, cpu>::compute(NumericTablePtr xTable, NumericTable *yTable, daal::algorithms::Model *r,
        ...
        size_t nActiveVectors, const char *I, size_t _nLines, size_t _lineSize, algorithmFPType *_cache, size_t *shrinkingRowIndices)
{
    ...
    while(i < j)
    {
        ...
        while ( (I[j] & shrink) && j > 0)                  j--;
        if (i >= j) break;
        daal::services::internal::swap<cpu, size_t>(shrinkingRowIndices[i], shrinkingRowIndices[j]);
        i++;
        j--;
    }
}
