...
Status Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    {
        return Status(services::Error::create(ErrorModelNotFullInitialized, services::ArgumentName, supportVectorsStr()));
    }
    if(!m->getClassificationCoefficients())
    {
        return Status(services::Error::create(ErrorModelNotFullInitialized, services::ArgumentName, classificationCoefficientsStr()));
    }
    return s;
}
