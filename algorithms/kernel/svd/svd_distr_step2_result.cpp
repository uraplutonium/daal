...
Status DistributedPartialResult::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    for(size_t i = 0 ; i < nNodes ; i++)
    {
        ...
        for(size_t j = 0 ; j < nodeSize ; j++)
        {
            ...
            DAAL_CHECK_STATUS(s, checkNumericTable(rNumTableInNodeCollection.get(), SVDNodeCollectionNTStr(), unexpectedLayouts, 0, nFeatures, nFeatures));
        }
    }
    Parameter *svdPar   = static_cast<Parameter *>(const_cast<daal::algorithms::Parameter *>(parameter  ));
    int unexpectedLayouts = (int)packed_mask;
    DAAL_CHECK_STATUS(s, checkNumericTable(get(finalResultFromStep2Master)->get(singularValues).get(), singularValuesStr(), unexpectedLayouts, 0, nFeatures, 1));
    ...
    if(svdPar->rightSingularMatrix == requiredInPackedForm)
    {
        ...
        {
            DAAL_CHECK_STATUS(s, checkNumericTable(get(finalResultFromStep2Master)->get(rightSingularMatrix).get(), rightSingularMatrixStr(),
