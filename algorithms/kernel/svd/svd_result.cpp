...
Status Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    Status s;
    DAAL_CHECK_STATUS(s, checkNumericTable(get(singularValues).get(), singularValuesStr(),
        unexpectedLayouts, 0, nFeatures, 1));
    if(svdPar->rightSingularMatrix == requiredInPackedForm)
    {
        DAAL_CHECK_STATUS(s, checkNumericTable(get(rightSingularMatrix).get(), rightSingularMatrixStr(), unexpectedLayouts, 0, nFeatures, nFeatures));
    }
    if(svdPar->leftSingularMatrix == requiredInPackedForm)
    {
        DAAL_CHECK_STATUS(s, checkNumericTable(get(leftSingularMatrix).get(), leftSingularMatrixStr(), unexpectedLayouts, 0, nFeatures, nVectors));
    }
    return s;
}
/**
...
Status Result::check(const daal::algorithms::PartialResult *pres, const daal::algorithms::Parameter *par, int method) const
{
    ...
    Status s;
    DAAL_CHECK_STATUS(s, checkNumericTable(get(singularValues).get(), singularValuesStr(), unexpectedLayouts, 0, nFeatures, 1));
    if(svdPar->rightSingularMatrix == requiredInPackedForm)
    {
        DAAL_CHECK_STATUS(s, checkNumericTable(get(rightSingularMatrix).get(), rightSingularMatrixStr(), unexpectedLayouts, 0, nFeatures, nFeatures));
    }
