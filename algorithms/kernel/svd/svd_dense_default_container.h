...
Status OnlineContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    Status s = partialResult->addPartialResultStorage<algorithmFPType>(m, n, kmPar);
    if(!s)
        return s;
...
Status OnlineContainer<algorithmFPType, method, cpu>::finalizeCompute()
{
    ...
    Status st = __DAAL_CALL_KERNEL_STATUS(env, internal::SVDOnlineKernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method), finalizeCompute, na, a, nr, r, &kmPar);
...
template<typename algorithmFPType, Method method, CpuType cpu>
Status DistributedContainer<step2Master, algorithmFPType, method, cpu>::finalizeCompute() { return Status(); }
...
Status DistributedContainer<step3Local, algorithmFPType, method, cpu>::compute()
{
    ...
    Status s = __DAAL_CALL_KERNEL_STATUS(env, internal::SVDDistributedStep3Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method), compute, na, a, nr, r, par);
...
template<typename algorithmFPType, Method method, CpuType cpu>
Status DistributedContainer<step3Local, algorithmFPType, method, cpu>::finalizeCompute() { return Status(); }
