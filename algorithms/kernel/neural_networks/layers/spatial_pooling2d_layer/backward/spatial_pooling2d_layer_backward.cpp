...
void Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if (gradientDims[index0] > gradientDims[index1]) { DAAL_CHECK_EX(one << pyramidHeight <= gradientDims[index0], services::ErrorIncorrectParameter, services::ParameterName, pyramidHeightStr()); }
    if (gradientDims[index0] < gradientDims[index1]) { DAAL_CHECK_EX(one << pyramidHeight <= gradientDims[index1], services::ErrorIncorrectParameter, services::ParameterName, pyramidHeightStr()); }
}
