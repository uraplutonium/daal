...
void Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    {
        DAAL_CHECK_EX(one << pyramidHeight <= dataDims[index0], services::ErrorIncorrectParameter, services::ParameterName, pyramidHeightStr());
    }
    if (dataDims[index0] < dataDims[index1])
    {
        DAAL_CHECK_EX(one << pyramidHeight <= dataDims[index1], services::ErrorIncorrectParameter, services::ParameterName, pyramidHeightStr());
    }
