...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    internal::PReLUTask<algorithmFPType, method, cpu> task(xTensor, wTensor, wDerTensor, parameter);

    __DAAL_CALL_KERNEL(env, internal::PReLUKernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method), compute, task, inGradTensor, xTensor, resultTensor, parameter);
}
} // namespace interface1
} // namespace backward
