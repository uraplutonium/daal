 ...
 */
services::Status Input::addData(const services::SharedPtr<data_management::Tensor> &dataTensor, size_t index)
{
...
services::Status Input::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    services::Status s;
    DAAL_CHECK_STATUS(s, data_management::checkTensor(layerDataTensor0.get(), inputLayerDataStr()));

    services::Collection<size_t> dims = layerDataTensor0->getDimensions();

    if (concatDimension > dims.size() - 1) return services::Status(services::ErrorIncorrectParameter);
    ...
    for (size_t i = 1; i < nInputs; i++)
    {
        ...
        DAAL_CHECK_STATUS(s, data_management::checkTensor(layerDataTensor.get(), inputLayerDataStr(), &dims));
    }
    return s;
}
                   ...
                   int method) const
{
    ...
    if(!algParameter->predictionStage)
    {
        ...
        if (dimsNT->getNumberOfColumns() != inputSize) return services::Status(services::ErrorIncorrectNumberOfColumnsInOutputNumericTable);
    ...
    return data_management::checkTensor(get(layers::forward::value).get(), valueStr(), &dims);
}
