...
void LRNKernel<algorithmFPType, method, cpu>::compute(const lrn::backward::Input *input, const lrn::Parameter *parameter,
        lrn::backward::Result *result)
{
    ...
    SharedPtr<Tensor> dataTable = input->get(layers::lrn::auxData);
    SharedPtr<Tensor> sMinusBetaTable = input->get(lrn::auxSmBeta);
    SharedPtr<Tensor> inputGradientTable = input->get(layers::backward::inputGradient);
    SharedPtr<Tensor> gradientTable = result->get(layers::backward::gradient);

    const services::Collection<size_t> &dims = dataTable->getDimensions();
    size_t nInputRows = dims[0];
    size_t nElements = dataTable->getSize();
    ...
    inputGradientTable->getSubtensor(0, 0, 0, nInputRows, readOnly, inputGradientBlock);
    algorithmFPType *inputGradientArray = inputGradientBlock.getPtr();

    SubtensorDescriptor<algorithmFPType> gradientBlock;
    gradientTable->getSubtensor(0, 0, 0, nInputRows, writeOnly, gradientBlock);
    ...
    if(0.0 == beta)
    {
        ...
        gradientTable->releaseSubtensor(gradientBlock);
        return;
    }
    ...
    dataTable->getSubtensor(0, 0, 0, nInputRows, readOnly, dataBlock);
    algorithmFPType *dataArray = dataBlock.getPtr();

    SubtensorDescriptor<algorithmFPType> sMinusBetaBlock;
    sMinusBetaTable->getSubtensor(0, 0, 0, nInputRows, readOnly, sMinusBetaBlock);
    ...
    vPowx<cpu>(nElements, sMinusBetaArray, (-beta - 1) / -beta, sMBetaM1Array);
    ...
    dataTable->releaseSubtensor(dataBlock);
    inputGradientTable->releaseSubtensor(inputGradientBlock);
    gradientTable->releaseSubtensor(gradientBlock);
