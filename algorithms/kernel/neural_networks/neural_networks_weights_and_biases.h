...
public:
    ...
    services::Status setTable(const services::SharedPtr<data_management::HomogenNumericTable<modelFPType> > &table)
    {
        _wbTable = table;
        return services::Status();
    }

    virtual services::Status copyFromTable(const data_management::NumericTablePtr &table) DAAL_C11_OVERRIDE
    {
        using namespace services;
        using namespace data_management;

        if (table.get() == _wbTable.get()) return services::Status();
    ...
    virtual services::Status copyFromTable(const data_management::NumericTablePtr &table, size_t idx) DAAL_C11_OVERRIDE
    {
        using namespace services;
        using namespace data_management;
...
protected:
    ...
    services::Status computeWeightsAndBiasesSizes(const services::SharedPtr<neural_networks::ForwardLayers> &forwardLayers)
    {
        using namespace services;
        using namespace data_management;
...
protected:
    ...
    services::Status tableToTensors(const data_management::NumericTablePtr &table,
