    ...
    const Input *input, const neural_networks::training::Parameter *parameter, Result *result)
{
    ...
    SharedPtr<iterative_solver::Result> iterativeSolverResult = SharedPtr<iterative_solver::Result>(new iterative_solver::Result());
    iterativeSolverResult->set(iterative_solver::nIterations, nIterations);
    iterativeSolverAlgorithm->setResult(iterativeSolverResult);
    ...
    {
        SharedPtr<NumericTable> weightsAndBiases = nnModel->getAllWeightsAndBiases<algorithmFPType>();
        SharedPtr<NumericTable> weightsAndBiasesDerivatives = nnModel->getAllWeightsAndBiasesDerivatives<algorithmFPType>();
    ...
    loss::forward::Input *lossInput = static_cast<loss::forward::Input *>(forwardLayers->get(nLayers - 1)->getLayerInput());
    lossInput->set(loss::forward::groundTruth, sampleGroundTruth);
    forwardLayers->get(nLayers - 1)->getLayerResult()->setResultForBackward(lossInput);
    ...
    for(size_t i = 0; i < nSamples; i += batchSize)
    {
        ...
        if (oneTableForAllWeights)
        {
            iterativeSolverAlgorithm->compute();
        }
        else
        {
            for(size_t layerId = 0; layerId < nLayers; layerId++)
            {
                SharedPtr<NumericTable> weightsAndBiases = nnModel->getWeightsAndBiases<algorithmFPType>(layerId);
                if (weightsAndBiases)
                {
                    SharedPtr<NumericTable> weightsAndBiasesDerivatives = nnModel->getWeightsAndBiasesDerivatives<algorithmFPType>(layerId);
                    precomputedCollection->get(objective_function::gradientIdx) = weightsAndBiasesDerivatives;
                    iterativeSolverAlgorithm->input.set(iterative_solver::inputArgument, weightsAndBiases);
                    iterativeSolverResult->set(iterative_solver::minimum, weightsAndBiases);
                    ...
                    nnModel->setWeightsAndBiases<algorithmFPType>(layerId, weightsAndBiases);
                }
            }
        }
