...
services::Status NeuralNetworksFeedforwardPredictionKernel<algorithmFPType, method, cpu>::initialize(
    const Input *input, const neural_networks::prediction::Parameter *parameter, Result *result)
{
    ...
    if (nSamples < batchSize) { return services::Status(); }
 ...
 */
template<typename algorithmFPType, Method method, CpuType cpu>
services::Status NeuralNetworksFeedforwardPredictionKernel<algorithmFPType, method, cpu>::compute(const Input *input, Result *result)
{
    SharedPtr<ForwardLayers> forwardLayers = input->get(prediction::model)->getLayers();
    TensorPtr data = input->get(prediction::data);
    if (nSamples < batchSize) { DAAL_RETURN_STATUS() }
...
template<typename algorithmFPType, Method method, CpuType cpu>
services::Status NeuralNetworksFeedforwardPredictionKernel<algorithmFPType, method, cpu>::reset()
{
