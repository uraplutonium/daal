...
Status Input::check(const daal::algorithms::Parameter *par, int method) const
{
    ...
    size_t nColumns = 2;
    Status s = checkNumericTable(get(data).get(), dataStr(), unexpectedLayouts, 0, nColumns);
    if(!s) { return s; }
...
Status Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const
{
    ...
    s |= checkNumericTable(get(largeItemsets).get(),        largeItemsetsStr(),        unexpectedLayouts, 0, 2, 0, false);
    if(!s) return s;
    s |= checkNumericTable(get(largeItemsetsSupport).get(), largeItemsetsSupportStr(), unexpectedLayouts, 0, 2, 0, false);
    if(!s) return s;

    if(algParameter->discoverRules)
    {
        DAAL_CHECK(Argument::size() == 5, ErrorIncorrectNumberOfOutputNumericTables);
        s |= checkNumericTable(get(antecedentItemsets).get(), antecedentItemsetsStr(), unexpectedLayouts, 0, 2, 0, false);
        if(!s) return s;
        s |= checkNumericTable(get(consequentItemsets).get(), consequentItemsetsStr(), unexpectedLayouts, 0, 2, 0, false);
        if(!s) return s;
        s |= checkNumericTable(get(confidence).get(), confidenceStr(), unexpectedLayouts, 0, 1, 0, false);
        if(!s) return s;
    }
    else
    {
