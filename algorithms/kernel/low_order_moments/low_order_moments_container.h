...
{
    __DAAL_INTIALIZE_KERNELS(internal::LowOrderMomentsBatchKernel, algorithmFPType, method);
}
...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    SharedPtr<NumericTable> dataTable = input->get(data);
...
{
    __DAAL_INTIALIZE_KERNELS(internal::LowOrderMomentsOnlineKernel, algorithmFPType, method);
}
...
void OnlineContainer<algorithmFPType, method, cpu>::finalizeCompute()
{
    ...
    SharedPtr<NumericTable> nObservationsTable = partialResult->get(nObservations);
    ...
    SharedPtr<NumericTable> meanTable      = result->get(mean);
    SharedPtr<NumericTable> raw2MomTable   = result->get(secondOrderRawMoment);
    SharedPtr<NumericTable> varianceTable  = result->get(variance);
    SharedPtr<NumericTable> stDevTable     = result->get(standardDeviation);
    SharedPtr<NumericTable> variationTable = result->get(variation);
...
{
    __DAAL_INTIALIZE_KERNELS(internal::LowOrderMomentsDistributedKernel, algorithmFPType, method);
}
...
void DistributedContainer<step2Master, algorithmFPType, method, cpu>::compute()
{
    ...
    services::SharedPtr<data_management::DataCollection> collection = input->get(low_order_moments::partialResults);
