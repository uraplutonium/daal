...
services::Status Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(method == fastCSR || method == singlePassCSR || method == sumCSR)
    {
        ...
        s |= checkNumericTable(dataTable.get(), dataStr(), 0, expectedLayout);
        if(!s) return s;
    }
    else
    {
        s |= checkNumericTable(dataTable.get(), dataStr());
        if(!s) return s;
    }
    if(method == sumDense || method == sumCSR)
    {
        NumericTablePtr sum = dataTable->basicStatistics.get(NumericTableIface::sum);
        s |= checkNumericTable(sum.get(), basicStatisticsSumStr(), 0, 0, dataTable->getNumberOfColumns(), 1);
    }
    return s;
}
