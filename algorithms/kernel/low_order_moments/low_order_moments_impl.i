...
LowOrderMomentsBatchTask<algorithmFPType, cpu>::LowOrderMomentsBatchTask(
            SharedPtr<NumericTable> &dataTable, Result *result) :
    dataTable(dataTable)
{
...
LowOrderMomentsOnlineTask<algorithmFPType, cpu>::LowOrderMomentsOnlineTask(
            SharedPtr<NumericTable> &dataTable, PartialResult *partialResult, bool isOnline,
...
template<typename algorithmFPType, CpuType cpu>
LowOrderMomentsFinalizeTask<algorithmFPType, cpu>::LowOrderMomentsFinalizeTask(SharedPtr<NumericTable> &nObservationsTable,
            SharedPtr<NumericTable> &sumTable, SharedPtr<NumericTable> &sumSqTable, SharedPtr<NumericTable> &sumSqCenTable,
            SharedPtr<NumericTable> &meanTable, SharedPtr<NumericTable> &raw2MomTable, SharedPtr<NumericTable> &varianceTable,
            SharedPtr<NumericTable> &stDevTable, SharedPtr<NumericTable> &variationTable) :
...
void retrievePrecomputedStatsIfPossible(size_t nFeatures, size_t nVectors,
            SharedPtr<NumericTable> &dataTable,
            algorithmFPType *sums, algorithmFPType *mean,
            services::SharedPtr<services::KernelErrorCollection> &_errors)
{
    SharedPtr<NumericTable> precomputedSumsTable = dataTable->basicStatistics.get(NumericTable::sum);
...
template<typename algorithmFPType, CpuType cpu>
void mergeNObservations(services::SharedPtr<data_management::DataCollection> &partialResultsCollection,
            PartialResult *partialResult, int *partialNObservations)
{
    SharedPtr<PartialResult> inputPartialResult = staticPointerCast<PartialResult, SerializationIface>(
            (*partialResultsCollection)[0]);

    SharedPtr<NumericTable> nObsTable = partialResult->get(nObservations);
    SharedPtr<NumericTable> inputNObsTable = inputPartialResult->get(nObservations);
    ...
    {
        inputPartialResult = staticPointerCast<PartialResult, SerializationIface>(
            (*partialResultsCollection)[i]);
        inputNObsTable = inputPartialResult->get(nObservations);
        inputNObsTable->getBlockOfRows(0, 1, readOnly, inputNObsBD);
        int *inputNObs = inputNObsBD.getBlockPtr();
...
void getTwoTables(ReadWriteMode rwMode,
            SharedPtr<NumericTable> &table1, SharedPtr<NumericTable> &table2,
...
template<typename algorithmFPType, CpuType cpu>
void mergeMinAndMax(services::SharedPtr<data_management::DataCollection> &partialResultsCollection,
            PartialResult *partialResult)
{
    SharedPtr<NumericTable> minTable = partialResult->get(partialMinimum);
    SharedPtr<NumericTable> maxTable = partialResult->get(partialMaximum);
    ...
    SharedPtr<PartialResult> inputPartialResult = staticPointerCast<PartialResult, SerializationIface>(
            (*partialResultsCollection)[0]);
    SharedPtr<NumericTable> inputMinTable = inputPartialResult->get(partialMinimum);
    SharedPtr<NumericTable> inputMaxTable = inputPartialResult->get(partialMaximum);
    ...
    for (size_t i = 1; i < collectionSize; i++)
    {
        inputPartialResult = staticPointerCast<PartialResult, SerializationIface>(
            (*partialResultsCollection)[i]);
        inputMinTable = inputPartialResult->get(partialMinimum);
        inputMaxTable = inputPartialResult->get(partialMaximum);
...
void getThreeTables(ReadWriteMode rwMode,
            SharedPtr<NumericTable> &table1, SharedPtr<NumericTable> &table2, SharedPtr<NumericTable> &table3,
            ...
            PartialResult *partialResult, int *partialNObservations)
{
    ...
    SharedPtr<NumericTable> sumTable      = partialResult->get(partialSum);
    SharedPtr<NumericTable> sumSqTable    = partialResult->get(partialSumSquares);
    SharedPtr<NumericTable> sumSqCenTable = partialResult->get(partialSumSquaresCentered);
    ...
    SharedPtr<PartialResult> inputPartialResult = staticPointerCast<PartialResult, SerializationIface>(
            (*partialResultsCollection)[0]);
    SharedPtr<NumericTable> inputSumTable      = inputPartialResult->get(partialSum);
    SharedPtr<NumericTable> inputSumSqTable    = inputPartialResult->get(partialSumSquares);
    SharedPtr<NumericTable> inputSumSqCenTable = inputPartialResult->get(partialSumSquaresCentered);
    ...
    {
        inputPartialResult = staticPointerCast<PartialResult, SerializationIface>(
            (*partialResultsCollection)[block]);
        inputSumTable      = inputPartialResult->get(partialSum);
        inputSumSqTable    = inputPartialResult->get(partialSumSquares);
        inputSumSqCenTable = inputPartialResult->get(partialSumSquaresCentered);
...
void finalize(LowOrderMomentsFinalizeTask<algorithmFPType, cpu> &task)
{
    ...
    vSqrt<cpu>(nFeatures, variance, stDev);
