...
services::Status Result::check(const daal::algorithms::PartialResult *partialResult, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    if(resModel->getLogP()->getNumberOfColumns() != 1) return services::Status(services::ErrorIncorrectSizeOfModel);
    if(resModel->getLogP()->getNumberOfRows() != nClasses) return services::Status(services::ErrorIncorrectSizeOfModel);

    if(resModel->getLogTheta()->getNumberOfColumns() != nFeatures) return services::Status(services::ErrorIncorrectSizeOfModel);
    if(resModel->getLogTheta()->getNumberOfRows() != nClasses) return services::Status(services::ErrorIncorrectSizeOfModel);
