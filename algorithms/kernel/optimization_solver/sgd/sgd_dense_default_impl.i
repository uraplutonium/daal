...
void SGDKernel<algorithmFPType, defaultDense, cpu>::compute(iterative_solver::Input *input, iterative_solver::Result *result, Parameter<defaultDense> *parameter)
{
    size_t nFeatures = input->get(iterative_solver::inputArgument)->getNumberOfColumns();
    BlockMicroTable<algorithmFPType, writeOnly, cpu> mtWorkValue(result->get(iterative_solver::minimum).get());
    algorithmFPType *workValue;
    mtWorkValue.getBlockOfRows(0, 1, &workValue);
    BlockMicroTable<algorithmFPType, readOnly, cpu> mtStartValue(input->get(iterative_solver::inputArgument).get());
    ...
    if(maxIterations == 0)
    {
        BlockMicroTable<int, writeOnly, cpu> mtNIterations(result->get(iterative_solver::nIterations).get());
