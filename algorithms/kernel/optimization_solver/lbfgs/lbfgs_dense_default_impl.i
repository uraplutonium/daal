    ...
    OptionalArgument *optionalArgumentResult, Parameter *parameter)
{
    ...
    for (; t < nLIterations;)
    {
        ...
        if (t >= 2)
        {
            ...
            services::Status s = hessianFunction->computeNoThrow();
            if(!s) { task.setToResult(correctionIndicesResult, nIterationsNT, optionalArgumentResult,
                                                                             curIteration, epoch, correctionIndex); DAAL_RETURN_STATUS() }
