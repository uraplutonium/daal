...
public:
    ...
    template<typename T>
    services::Status releaseTBlock(size_t idx, size_t nrows, void *buf, ReadWriteMode rwFlag,
    ...
    template<typename T>
    services::Status releaseTFeature(size_t feature_idx, size_t idx, size_t nrows, void *buf, ReadWriteMode rwFlag,
                         const char *javaMethodName)
    {
        jint status = JNI_OK;
        if(rwFlag == writeOnly)
