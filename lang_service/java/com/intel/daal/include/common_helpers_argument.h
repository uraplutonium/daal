...
struct jniInput
{
    ...
    static jlong get( jlong inputAddr, jint id, Args&&... args)
    {
        ...
        SharedPtr<SerializationIface> * dShPtr = new SharedPtr<SerializationIface>();
    ...
    {
        _Input *input = (_Input*)inputAddr;
        SharedPtr<SerializationIface> *dataShPtr = (SharedPtr<SerializationIface> *)dataAddr;
...
struct jniArgument
{
    static jlong newObj()
    {
        return (jlong)( new services::SharedPtr<SerializationIface>( new _Argument() ) );
    }

    template<typename _IdType, typename _DataType, typename... Args>
    static jlong get( jlong argumentAddr, jint id, Args&&... args )
    {
        SharedPtr<_Argument> argument = staticPointerCast<_Argument, SerializationIface>( *(SharedPtr<SerializationIface> *)argumentAddr );
        SharedPtr<SerializationIface> * dShPtr = new SharedPtr<SerializationIface>();
