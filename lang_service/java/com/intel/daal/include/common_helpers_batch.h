...
struct jniBatch
{
    ...
    static jlong getParameterValue( jint prec, int method, jlong algAddr )
    {
        if( method==head ) return jniBatch<_Method, _AlgClass, head     >::getParameterValue( prec, method, algAddr );
        else               return jniBatch<_Method, _AlgClass, values...>::getParameterValue( prec, method, algAddr );
    }
...
struct jniBatch<_Method, _AlgClass, head>
{
    ...
    static void setResult( jint prec, int method, jlong algAddr, jlong resAddr )
    {
        SharedPtr<SerializationIface> *serializableShPtr = (services::SharedPtr<SerializationIface> *)resAddr;
