 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_com_intel_daal_algorithms_neural_1networks_prediction_PredictionModel_cInitFromLayerDescriptors
  (JNIEnv *env, jobject thisObj, jlong layerDescriptorsAddr)
{
     SharedPtr<Collection<layers::LayerDescriptor> > layerDescriptors =
        *(SharedPtr<Collection<layers::LayerDescriptor> > *)layerDescriptorsAddr;
    return (jlong)(new SharedPtr<prediction::Model>(new prediction::Model(*(layerDescriptors.get()))));
}
