  ...
  (JNIEnv *env, jobject thisObj, jlong cModel, jint prec, jlongArray dataSizeArray, jlong layerDescriptorsAddr, jlong parameterAddr)
{
    ...
    if (prec == 0)
    {
        model->initialize<double>(dataSizeCollection, *layerDescriptors, (training::Parameter *)parameterAddr);
    } else
    {
        model->initialize<float>(dataSizeCollection, *layerDescriptors, (training::Parameter *)parameterAddr);
    }
}
