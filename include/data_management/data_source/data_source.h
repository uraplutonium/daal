...
protected:
    ...
    void setNumericTableDictionary(services::SharedPtr<NumericTable> nt)
    {
...
public:
    ...
    services::Status resizeNumericTableImpl( size_t linesToLoad, NumericTable* nt)
    {
        ...
        if (nt->getNumberOfColumns() < nFeatures) {
            nt->getDictionary()->setNumberOfFeatures(nFeatures);
        }
        ...
        if( ntMin->getNumberOfColumns() != nCols || ntMin->getNumberOfRows() != 1 )
        {
            ...
            {
                ntMin->getDictionary()->setNumberOfFeatures(nCols);
            }
            ntMin->resize(1);
        }
        ...
        if( ntMax->getNumberOfColumns() != nCols || ntMax->getNumberOfRows() != 1 )
        {
            ...
            {
                ntMax->getDictionary()->setNumberOfFeatures(nCols);
            }
            ntMax->resize(1);
        }
        ...
        if( ntSum->getNumberOfColumns() != nCols || ntSum->getNumberOfRows() != 1 )
        {
            ...
            {
                ntSum->getDictionary()->setNumberOfFeatures(nCols);
            }
            ntSum->resize(1);
        }
        ...
        if( ntSumSq->getNumberOfColumns() != nCols || ntSumSq->getNumberOfRows() != 1 )
        {
            ...
            {
                ntSumSq->getDictionary()->setNumberOfFeatures(nCols);
            }
            ntSumSq->resize(1);
        }
        return services::Status();
