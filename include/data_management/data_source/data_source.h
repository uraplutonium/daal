...
{
/**
 *  <a name="DAAL-CLASS-DATASOURCEIFACE"></a>
...
public:
    /**
     * <a name="DAAL-ENUM-DATASOURCEIFACE_DATASOURCESTATUS"></a>
    ...
    /**
     * <a name="DAAL-ENUM-DATASOURCEIFACE_DICTIONARYCREATIONFLAG"></a>
    ...
    /**
     * <a name="DAAL-ENUM-DATASOURCEIFACE_NUMERICTABLEALLOCATIONFLAG"></a>
...
public:
     ...
     */
    virtual services::SharedPtr<NumericTable> &getNumericTable() = 0;
...
/**
 *  <a name="DAAL-CLASS-DATASOURCE"></a>
...
public:
    ...
    services::SharedPtr<NumericTable> &getNumericTable() DAAL_C11_OVERRIDE
    {
        checkNumericTable();
    ...
    {
        if( _spnt.get() != NULL && _dict != NULL )
        {
            return services::SharedPtr<services::ErrorCollection>(new services::ErrorCollection(_errors, _spnt->getErrors(), _dict->getErrors()));
        }
        if( _spnt.get() != NULL )
        {
            return services::SharedPtr<services::ErrorCollection>(new services::ErrorCollection(_errors, _spnt->getErrors()));
        }
        if( _dict != NULL )
        {
            return services::SharedPtr<services::ErrorCollection>(new services::ErrorCollection(_errors, _dict->getErrors()));
        }

        return services::SharedPtr<services::ErrorCollection>(new services::ErrorCollection(_errors));
    }

protected:
    DataSourceDictionary    *_dict;
    services::SharedPtr<NumericTable> _spnt;
...
public:
    ...
    void allocateNumericTable() DAAL_C11_OVERRIDE
    {
        ...
        _spnt = services::SharedPtr<NumericTable>(nt);

        HomogenNumericTable<_summaryStatisticsType> *ssNt = 0;

        allocateNumericTableImpl( &ssNt );
        _spnt->basicStatistics.set(NumericTable::minimum  , services::SharedPtr<NumericTable>(ssNt));

        allocateNumericTableImpl( &ssNt );
        _spnt->basicStatistics.set(NumericTable::maximum  , services::SharedPtr<NumericTable>(ssNt));

        allocateNumericTableImpl( &ssNt );
        _spnt->basicStatistics.set(NumericTable::sum      , services::SharedPtr<NumericTable>(ssNt));

        allocateNumericTableImpl( &ssNt );
        _spnt->basicStatistics.set(NumericTable::sumSquares, services::SharedPtr<NumericTable>(ssNt));

    }

    void freeNumericTable() DAAL_C11_OVERRIDE
    {
        _spnt = services::SharedPtr<NumericTable>();
    }
    ...
    void resizeNumericTableImpl( size_t linesToLoad, NumericTable* nt)
    {
        ...
        services::SharedPtr<NumericTable> ntMin   = nt->basicStatistics.get(NumericTable::minimum   );
        services::SharedPtr<NumericTable> ntMax   = nt->basicStatistics.get(NumericTable::maximum   );
        services::SharedPtr<NumericTable> ntSum   = nt->basicStatistics.get(NumericTable::sum       );
        services::SharedPtr<NumericTable> ntSumSq = nt->basicStatistics.get(NumericTable::sumSquares);
    ...
    void combineSingleStatistics ( NumericTable *ntSrc, NumericTable *ntDst, bool wasEmpty, NumericTable::BasicStatisticsId id) {
        ...
        services::SharedPtr<NumericTable> ntSrcStat = ntSrc->basicStatistics.get(id);
        services::SharedPtr<NumericTable> ntDstStat = ntDst->basicStatistics.get(id);
