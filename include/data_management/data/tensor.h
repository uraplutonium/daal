...
/**
 *  <a name="DAAL-CLASS-TENSORLAYOUTIFACE"></a>
...
/**
 *  <a name="DAAL-CLASS-TENSORLAYOUT"></a>
...
/**
 *  <a name="DAAL-CLASS-TENSOROFFSETLAYOUT"></a>
...
public:
     ...
     */
    TensorOffsetLayout(const services::Collection<size_t>& dims) : TensorLayout(dims), _offsets(dims.size())
    {
        if(_nDims==0) return;
    ...
    TensorOffsetLayout(const services::Collection<size_t>& dims, const services::Collection<size_t>& offsets) : TensorLayout(dims)
    {
        ...
        int defaultLayoutMiss = (_offsets[lastIndex] == 1);
        for(size_t i=1; i<_nDims; i++)
        {
            defaultLayoutMiss += (_offsets[lastIndex-i] == _offsets[lastIndex-i+1]*_dims[lastIndex-i+1]);
        }

        if( defaultLayoutMiss==0 )
        {
            _layout = TensorIface::defaultLayout;
        }
        else
