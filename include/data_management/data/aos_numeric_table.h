...
private:
    ...
    template <typename T>
    services::Status releaseTBlock( size_t idx, size_t nrows, T *buf, int rwFlag )
    {
    ...
    template <typename T>
    services::Status releaseTFeature( size_t feat_idx, size_t idx, size_t nrows, T *buf, int rwFlag )
    {
        if (rwFlag & (int)writeOnly)
        {
            NumericTableFeature &f = (*_ddict)[feat_idx];
