...
public:
    ...
    services::Status initialize(const services::Collection<size_t> &dataSize, const Topology &topology,
     ...
     * \return Status of computations
     */
    services::Status setErrors(services::ErrorCollection &errors)
    {
        _errors = errors;
        return services::Status();
    }
     ...
     */
    template<typename modelFPType>
    services::Status allocate(const services::Collection<size_t> &dataSize, const daal::algorithms::Parameter *parameter)
    {
        using namespace services;
        using namespace data_management;
        using namespace layers;
        ...
        _sampleSize[0] = par->optimizationSolver->parameter->batchSize;
