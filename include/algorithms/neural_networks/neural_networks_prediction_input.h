...
public:
     ...
     */
    services::SharedPtr<data_management::Tensor> get(const TensorInputId &id) const
    {
     ...
     */
    void set(const TensorInputId &id, const services::SharedPtr<data_management::Tensor> &value)
    {
     ...
     * \return          %Input object that corresponds to the given identifier
     */
    services::SharedPtr<Model> get(const ModelInputId &id) const
    {
     ...
     * \param[in] value   Pointer to the parameter
     */
    void set(const ModelInputId &id, const services::SharedPtr<Model> &value)
    {
        Argument::set(id, value);
    }
    ...
    */
    void check(const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<Model> modelObject = get(model);

        if(dataTable.get() == 0)                 { this->_errors->add(services::ErrorNullInputNumericTable); return;         }
        if(modelObject.get() == 0)               { this->_errors->add(services::ErrorNullModel); return;         }
    }
};
} // namespace interface1
