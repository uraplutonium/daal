...
public:
    ...
    services::Status allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, const int method)
    {
        ...
        for (size_t i = 0; i < nLastLayers; i++)
        {
            ...
            {
                return services::Status(Error::create(services::ErrorNullTensor, ArgumentName, valueStr()));
            }
    ...
    services::Status check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        using namespace daal::data_management;
        using namespace daal::services;
        if(Argument::size() != 2) { return services::Status(services::ErrorIncorrectNumberOfOutputNumericTables); }

        KeyValueDataCollectionPtr predictionTensorCollection = get(predictionCollection);
        if (!predictionTensorCollection)
            return services::Status(ErrorNullOutputDataCollection);

        if (predictionTensorCollection->size() == 0)
            return services::Status(ErrorIncorrectNumberOfElementsInResultCollection);
        ...
        for (size_t i = 0; i < nLastLayers; i++)
        {
            ...
            if (!predictionTensor)
            {
                return services::Status(Error::create(services::ErrorNullTensor, ArgumentName, predictionStr()));
            }
