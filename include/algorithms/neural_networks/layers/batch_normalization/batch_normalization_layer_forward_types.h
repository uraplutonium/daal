...
public:
    ...
    */
    ...
    void allocate(const daal::algorithms::Parameter *parameter, const int method)
    {
        ...
        if (!get(layers::forward::weights))
        {
            set(layers::forward::weights, services::SharedPtr<data_management::Tensor>(
                    new data_management::HomogenTensor<algorithmFPType>(getWeightsSizes(param), data_management::Tensor::doAllocate, 0.5f)));
        }

        if (!get(layers::forward::biases))
        {
            set(layers::forward::biases, services::SharedPtr<data_management::Tensor>(
                    new data_management::HomogenTensor<algorithmFPType>(getBiasesSizes(param), data_management::Tensor::doAllocate, 0.0f)));
        }
    }
    ...
    void check(const daal::algorithms::Parameter *parameter, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (get(layers::forward::weights))
        {
        if (!data_management::checkTensor(get(layers::forward::weights).get(), this->_errors.get(), strWeights(), &weightDims)) { return; }
        }
        if (get(layers::forward::biases))
        {
        if (!data_management::checkTensor(get(layers::forward::biases).get(),  this->_errors.get(), strBiases(), &weightDims)) { return; }
        }
        if (!data_management::checkTensor(get(populationMean).get(),           this->_errors.get(), strPopulationMean(), &weightDims)) { return; }
        if (!data_management::checkTensor(get(populationVariance).get(),       this->_errors.get(), strPopulationVariance(), &weightDims)) { return; }
    }
};
...
public:
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(valueTensor.get(), this->_errors.get(), strValue(), &dataDims)) { return; }
        ...
        if (!data_management::checkTensor(get(auxWeights).get(),            this->_errors.get(), strAuxWeights(), &auxDims)) { return; }
        if (!data_management::checkTensor(get(auxMean).get(),               this->_errors.get(), strAuxMean(), &auxDims)) { return; }
        if (!data_management::checkTensor(get(auxStandardDeviation).get(),  this->_errors.get(), strAuxStandardDeviation(), &auxDims)) { return; }
        if (!data_management::checkTensor(get(auxPopulationMean).get(),     this->_errors.get(), strAuxPopulationMean(), &auxDims)) { return; }
        if (!data_management::checkTensor(get(auxPopulationVariance).get(), this->_errors.get(), strAuxPopulationVariance(), &auxDims)) { return; }
    }
