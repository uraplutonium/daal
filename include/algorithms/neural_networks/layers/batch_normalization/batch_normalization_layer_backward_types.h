...
public:
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(gradientTensor.get(), this->_errors.get(), strGradient(), &gradientDims)) { return; }
        ...
        if (!data_management::checkTensor(get(layers::backward::weightDerivatives).get(), this->_errors.get(), strWeightDerivatives(), &derDims)) { return; }
        if (!data_management::checkTensor(get(layers::backward::biasDerivatives).get(), this->_errors.get(), strBiasDerivatives(), &derDims)) { return; }
    }
