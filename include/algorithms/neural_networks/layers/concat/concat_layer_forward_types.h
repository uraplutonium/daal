...
public:
    ...
    */
    void check(const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<services::Error> error = checkTensor(layerDataTensor0, "inputLayerData");
        ...
        for (size_t i = 1; i < nInputs; i++)
        {
            ...
            error = checkTensor(layerDataTensor, "inputLayerData", &dims);
            if (error) { this->_errors->add(error); return; }
        }
    }
    ...
    */
    LayerInputLayout getLayout() { return collectionInput; }
};
...
public:
    ...
    */
                       ...
                       int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<services::Error> error = checkTensor(get(layers::forward::value), "value in Result");
        if (error) { this->_errors->add(error); return; }
        ...
        error = checkTensor(valueTensor, "value", &dims);
        if (error) { this->_errors->add(error); return; }
    }

    /**
    * Allocates memory for storing final results of the forward concat layer
    ...
    */
    int getSerializationTag() { return SERIALIZATION_NEURAL_NETWORKS_LAYERS_CONCAT_FORWARD_RESULT_ID; }
