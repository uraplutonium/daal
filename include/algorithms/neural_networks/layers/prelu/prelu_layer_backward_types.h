...
#ifndef __NEURAL_NETWORKS__PRELU_LAYER_BACKWARD_TYPES_H__
#define __NEURAL_NETWORKS__PRELU_LAYER_BACKWARD_TYPES_H__
...
public:
    ...
    * Allocates memory to store the result of the backward prelu layer
    ...
    void allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, const int method)
    {
        ...
        services::SharedPtr<services::Error> error = checkTensor(valueTensor, "auxData");
        if (error) { this->_errors->add(error); return; }
        error = checkTensor(weightsTensor, "auxWeights");
        if (error) { this->_errors->add(error); return; }

        Argument::set(layers::backward::gradient, services::SharedPtr<data_management::SerializationIface>(
                          new data_management::HomogenTensor<algorithmFPType>(valueTensor->getDimensions(),
        ...
        if( get(layers::backward::weightDerivatives) == 0 )
        {
            Argument::set(layers::backward::weightDerivatives, services::SharedPtr<data_management::SerializationIface>(
                              new data_management::HomogenTensor<algorithmFPType>(weightsTensor->getDimensions(),
                                                                                  data_management::Tensor::doAllocate)));
        }
    }
     ...
     */
    int getSerializationTag() { return SERIALIZATION_NEURAL_NETWORKS_LAYERS_PRELU_BACKWARD_RESULT_ID; }
