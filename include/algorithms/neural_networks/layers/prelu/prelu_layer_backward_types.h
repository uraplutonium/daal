...
public:
    ...
    * Allocates memory to store the result of the backward prelu layer
    ...
    void allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, const int method)
    {
        ...
        if (!data_management::checkTensor(valueTensor.get(), this->_errors.get(), strAuxData())) { return; }
        if (!data_management::checkTensor(weightsTensor.get(), this->_errors.get(), strAuxWeights())) { return; }
