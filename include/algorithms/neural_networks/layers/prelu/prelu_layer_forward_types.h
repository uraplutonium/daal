...
#ifndef __NEURAL_NETWORKS__PRELU_LAYER_FORWARD_TYPES_H__
#define __NEURAL_NETWORKS__PRELU_LAYER_FORWARD_TYPES_H__
...
public:
    ...
    */
    ...
    void allocate(const daal::algorithms::Parameter *par, const int method)
    {
        ...
        {
            set(layers::forward::weights, services::SharedPtr<Tensor>(new HomogenTensor<algorithmFPType>(getWeightsSizes(parameter),
                                                                                                         Tensor::doAllocate, 0.5)) );
        }
        else
        {
            const services::Collection<size_t> &dims = dataTensor->getDimensions();
            services::Collection<size_t> wdims(wDimNumber);
            for(size_t i = wStartDim; i < wStartDim + wDimNumber; i++)
            {
                wdims[i - wStartDim] = dims[i];
            }

            services::SharedPtr<services::Error> error = checkTensor(get(layers::forward::weights), "weights", &wdims);
...
public:
    ...
    */
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        error = checkTensor(in->get(layers::forward::weights), "weights");
        if (error) { this->_errors->add(error); return; }
    ...
    */
    ...
    /**
    * Allocates memory to store the result of forward prelu layer
     * \param[in] input     %Input object for the algorithm
     * \param[in] par       %Parameter of forward fprelu layer
    ...
    */
     ...
     */
    int getSerializationTag() { return SERIALIZATION_NEURAL_NETWORKS_LAYERS_PRELU_FORWARD_RESULT_ID; }
