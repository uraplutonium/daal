...
struct Padding
{
     ...
     */
    Padding(size_t first = 2) { size[0] = first;}
    size_t size[1];
};
...
struct SpatialDimensions
{
     ...
     */
    SpatialDimensions(size_t first = 2) { size[0] = first;}
    size_t size[1];
};
...
struct Parameter: public layers::Parameter
{
     ...
     */
    Parameter(size_t index, size_t kernelSize = 2, size_t stride = 2, size_t padding = 2, bool predictionStage = false) :
        indices(index), kernelSize(kernelSize), stride(stride), padding(padding), predictionStage(predictionStage)
    {}
    ...
    KernelSize kernelSize;      /*!< Data structure representing the size of the 1D subtensor
                                     from which the element is computed */
    SpatialDimensions indices;  /*!< Indices of the dimension on which pooling is performed */
