...
public:
    ...
    void check(const daal::algorithms::Parameter *parameter, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (spatialDimension > dataDims.size() - 1)
        {
        ...
        if (kernelSize == 0 || kernelSize > dataDims[spatialDimension] + 2 * param->padding.size[0])
        {
            services::SharedPtr<services::Error> error(new services::Error());
            error->setId(services::ErrorIncorrectParameter);
...
public:
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(valueTensor.get(), this->_errors.get(), strValue(), &valueDims)) { return; }
...
protected:
    ...
    {
        dims[param->indices.size[0]] =
            computeValueDimension(dims[param->indices.size[0]], param->kernelSize.size[0], param->padding.size[0], param->stride.size[0]);
    }
};
