...
public:
    ...
    {
        services::SharedPtr<services::Error> error = checkTensor(get(auxMask), "auxMask");
        if (error) { this->_errors->add(error); return services::Collection<size_t>(); }
...
protected:
    virtual services::Collection<size_t> getInputGradientSize(const pooling1d::Parameter *parameter) const
    {
        ...
        inputDims[param->indices.size[0]] =
            computeInputDimension(inputDims[param->indices.size[0]], param->kernelSize.size[0], param->padding.size[0], param->stride.size[0]);
        return inputDims;
    }
...
public:
     ...
     */
    int getSerializationTag() { return SERIALIZATION_NEURAL_NETWORKS_LAYERS_MAXIMUM_POOLING1D_BACKWARD_RESULT_ID; }
