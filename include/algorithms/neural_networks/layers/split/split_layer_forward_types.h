...
#ifndef __NEURAL_NETWORKS__SPLIT_LAYER_FORWARD_TYPES_H__
#define __NEURAL_NETWORKS__SPLIT_LAYER_FORWARD_TYPES_H__
...
{
/**
 * <a name="DAAL-ENUM-ALGORITHMS__NEURAL_NETWORKS__LAYERS___SPLIT__FORWARD__RESULTLAYERDATAID"></a>
...
public:
    ...
    */
     ...
     */
    LayerResultLayout getLayout() { return collectionResult; }
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<services::Error> error = checkTensor(inputTensor, "data");
        if (error) { this->_errors->add(error); return; }
        services::Collection<size_t> inputDims = inputTensor->getDimensions();

        for (size_t i = 0; i < nOutputs; i++)
        {
            services::SharedPtr<data_management::Tensor> resultTensor = get(valueCollection, i);
            services::SharedPtr<services::Error> error = checkTensor(resultTensor, "resultLayerData", &inputDims);
            if (error) { this->_errors->add(error); return; }
        }
    }
    ...
    void allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, const int method)
    {
        ...
        for(size_t i = 0; i < nOutputs; i++)
        {
            (*resultCollection)[i] = services::SharedPtr<data_management::Tensor>(new data_management::HomogenTensor<algorithmFPType>(
                                                                                      in->get(layers::forward::data)->getDimensions(),
                                                                                      data_management::Tensor::doAllocate));
        }
        Argument::set(layers::forward::resultForBackward, services::staticPointerCast<data_management::SerializationIface, LayerData>
                      (resultCollection));
    }

    /**
    * Allocates memory for storing final results of the split layer
    ...
    */
    int getSerializationTag() { return SERIALIZATION_NEURAL_NETWORKS_LAYERS_SPLIT_FORWARD_RESULT_ID; }
