...
public:
    ...
    */
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(inputTensor.get(), this->_errors.get(), strData())) { return; }

        services::Collection<size_t> inputDims = inputTensor->getDimensions();

        for (size_t i = 0; i < nOutputs; i++)
        {
            if (!data_management::checkTensor(get(valueCollection, i).get(), this->_errors.get(), strValueCollection(), &inputDims)) { return; }
        }
    }
