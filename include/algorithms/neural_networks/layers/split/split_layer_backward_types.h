...
#ifndef __NEURAL_NETWORKS__SPLIT_LAYER_BACKWARD_TYPES_H__
#define __NEURAL_NETWORKS__SPLIT_LAYER_BACKWARD_TYPES_H__
...
{
/**
* <a name="DAAL-ENUM-ALGORITHMS__NEURAL_NETWORKS__LAYERS__SPLIT__BACKWARD___INPUTLAYERDATAID"></a>
...
public:
    ...
    */
    ...
    void check(const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<services::Error> error = checkTensor(inputTensor0, "inputGradientCollection");
        if (error) { this->_errors->add(error); return; }
        services::Collection<size_t> dims0 = inputTensor0->getDimensions();

        for (size_t i = 1; i < nInputs; i++)
        {
            services::SharedPtr<data_management::Tensor> inputTensor = get(inputGradientCollection, i);
            services::SharedPtr<services::Error> error = checkTensor(inputTensor, "inputGradientCollection", &dims0);
            if (error) { this->_errors->add(error); return; }
        }
    }
    ...
    */
    virtual LayerInputLayout getLayout() const { return collectionInput; }
};
...
public:
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<services::Error> error = checkTensor(get(layers::backward::gradient), "gradient", &dims);
        if (error) { this->_errors->add(error); return; }
    }
    ...
    */
    int getSerializationTag() { return SERIALIZATION_NEURAL_NETWORKS_LAYERS_SPLIT_BACKWARD_RESULT_ID; }
