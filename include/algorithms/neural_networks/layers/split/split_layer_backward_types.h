...
public:
    ...
    */
    ...
    void check(const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(inputTensor0.get(), this->_errors.get(), strInputGradientCollection()))
        {
        ...
        {
            if (!data_management::checkTensor(get(inputGradientCollection, i).get(), this->_errors.get(), strInputGradientCollection(), &dims0))
            {
                size_t inx = this->_errors->size();
...
public:
    ...
    void check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(get(layers::backward::gradient).get(), this->_errors.get(), strGradient(), &dims)) { return; }
    }
