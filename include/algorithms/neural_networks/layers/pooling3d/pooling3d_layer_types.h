...
struct KernelSize
{
     ...
     */
    KernelSize(size_t first = 2, size_t second = 2, size_t third = 2)
    {
        size[0] = first;
        size[1] = second;
        size[2] = third;
...
struct Stride
{
     ...
     */
    Stride(size_t first = 2, size_t second = 2, size_t third = 2)
    {
        size[0] = first;
        size[1] = second;
        size[2] = third;
...
struct Padding
{
     ...
     */
    Padding(size_t first = 2, size_t second = 2, size_t third = 2)
    {
        size[0] = first;
        size[1] = second;
        size[2] = third;
...
struct SpatialDimensions
{
     ...
     */
    SpatialDimensions(size_t first = 0, size_t second = 1, size_t third = 2)
    {
        size[0] = first;
        size[1] = second;
        size[2] = third;
...
struct Parameter: public layers::Parameter
{
    ...
    size_t firstKernelSize = 2, size_t secondKernelSize = 2, size_t thirdKernelSize = 2,
        ...
        indices(firstIndex, secondIndex, thirdIndex), kernelSize(firstKernelSize, secondKernelSize, thirdKernelSize),
        stride(firstStride, secondStride, thirdStride), padding(firstPadding, secondPadding, thirdPadding),
        predictionStage(predictionStage)
    {}

    Stride stride;              /*!< Data structure representing the intervals on which the subtensors for pooling are selected */
    Padding padding;            /*!< Data structure representing the number of data elements to implicitly add
                                     to each size of the three-dimensional subtensor on which pooling is performed */
    KernelSize kernelSize;      /*!< Data structure representing the size of the three-dimensional subtensor
                                     from which the maximum element is selected */
    SpatialDimensions indices;  /*!< Indices of the three dimensions on which pooling is performed */
    bool predictionStage;       /*!< Flag that specifies whether the layer is used for the prediction stage or not */
};
/* [Parameter source code] */
