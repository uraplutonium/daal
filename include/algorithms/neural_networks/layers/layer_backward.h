...
public:
     ...
     */
     virtual services::Status addInput(services::SharedPtr<backward::Result> result, size_t resultIndex, size_t inputIndex) = 0;
...
public:
     ...
     */
    virtual services::Status addInput(services::SharedPtr<backward::Result> result, size_t resultIndex, size_t inputIndex) DAAL_C11_OVERRIDE
    {
        return getLayerInput()->addInputGradient(result->getGradient(resultIndex), inputIndex);
    }
};

typedef services::SharedPtr<LayerIface> LayerIfaceImplPtr;
/** @} */
} // namespace interface1
