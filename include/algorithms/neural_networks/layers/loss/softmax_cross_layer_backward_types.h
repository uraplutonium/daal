...
public:
    ...
    void check(const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if (!data_management::checkTensor(get(auxProbabilities).get(), this->_errors.get(), strAuxProbabilities())) { return; }
        if (!data_management::checkTensor(get(auxGroundTruth).get(), this->_errors.get(), strAuxGroundTruth())) { return; }
    }
};
    ...
    void allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, const int method)
    {
        if (!get(layers::backward::gradient))
        {
            ...
            set(layers::backward::gradient, services::SharedPtr<data_management::Tensor>(
                              new data_management::HomogenTensor<algorithmFPType>(probabilitiesTable->getDimensions(),
