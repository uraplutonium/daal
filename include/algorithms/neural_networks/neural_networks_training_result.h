...
public:
    ...
    {
        if(Argument::size() != 1) return services::Status(services::ErrorIncorrectNumberOfOutputNumericTables);
        if(!get(model)) return services::Status(services::ErrorNullModel);
        return services::Status();
    }
