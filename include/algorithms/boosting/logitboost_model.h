...
{
    /** Default constructor */
    Parameter() : boosting::Parameter(), accuracyThreshold(0.0), maxIterations(10), nClasses(0),
    ...
    Parameter(services::SharedPtr<weak_learner::training::Batch>   wlTrain,
        ...
        accuracyThreshold(acc), maxIterations(maxIter), nClasses(nC), weightsDegenerateCasesThreshold(wThr) {}
...
public:
    ...
    Model(const Parameter *par, modelFPType dummy) : boosting::Model(), _nIterations(par->maxIterations)
    {}
    ...
    int getSerializationTag() { return SERIALIZATION_LOGITBOOST_MODEL_ID; }
    /**
