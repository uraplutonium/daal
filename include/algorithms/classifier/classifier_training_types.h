...
protected:
    ...
    void checkImpl(const daal::algorithms::Parameter *parameter) const
    {
        ...
        if(nRows < 1) { this->_errors->add(services::ErrorEmptyInputNumericTable); return; }
        if(dataTable->getNumberOfColumns() < 1)      { this->_errors->add(services::ErrorEmptyInputNumericTable); return; }

        services::SharedPtr<data_management::NumericTable> labelsTable = get(labels);
        if(!labelsTable) { this->_errors->add(services::ErrorNullInputNumericTable); return; }
        if(labelsTable->getNumberOfColumns() < 1)    { this->_errors->add(services::ErrorEmptyInputNumericTable); return; }
        if(nRows != labelsTable->getNumberOfRows())  { this->_errors->add(services::ErrorInconsistentNumberOfRows); return; }

        services::SharedPtr<data_management::NumericTable> weightsTable = get(weights);
        if(weightsTable)
        {
            if(weightsTable->getNumberOfColumns() < 1)   { this->_errors->add(services::ErrorEmptyInputNumericTable); return; }
            if(nRows != weightsTable->getNumberOfRows()) { this->_errors->add(services::ErrorInconsistentNumberOfRows); return; }
        }
    }
};
...
public:
    ...
    int getSerializationTag() { return SERIALIZATION_CLASSIFIER_TRAINING_PARTIAL_RESULT_ID; }
...
public:
    ...
    int getSerializationTag() { return SERIALIZATION_CLASSIFIER_TRAINING_RESULT_ID; }
...
public:
    ...
    virtual size_t getNumberOfFeatures() const
    {
        services::SharedPtr<data_management::DataCollection> models = get(classifier::training::partialModels);
     ...
     */
    void set(Step2MasterInputId id, const services::SharedPtr<data_management::DataCollection> &value)
    {
        Argument::set(id, value);
    }
    ...
    void check(const daal::algorithms::Parameter *parameter, int method) const DAAL_C11_OVERRIDE
    {
        services::SharedPtr<data_management::DataCollection> spModels = get(partialModels);
