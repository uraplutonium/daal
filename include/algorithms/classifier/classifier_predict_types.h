...
public:
    ...
    int getSerializationTag() { return SERIALIZATION_CLASSIFIER_PREDICTION_RESULT_ID; }
...
protected:
    void checkImpl(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter) const
    {
        ...
        services::SharedPtr<data_management::NumericTable> resTable = get(prediction);

        if(nRows != resTable->getNumberOfRows()) { this->_errors->add(services::ErrorInconsistentNumberOfRows); return; }
        if(resTable->getNumberOfColumns() < 1) { this->_errors->add(services::ErrorEmptyInputNumericTable); return; }
    }
