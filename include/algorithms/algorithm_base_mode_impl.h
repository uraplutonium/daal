...
public:
    ...
    services::Status finalizeComputeNoThrow()
    {
        if(this->isChecksEnabled())
        {
            services::Status _s = this->checkPartialResult();
            if(!_s)
                return _s;
        }
        ...
        if(this->isChecksEnabled())
        {
            services::Status _s = this->checkFinalizeComputeParams();
            if(!_s)
                return _s;
        }
...
public:
    ...
    */
    ...
    virtual services::Status checkResult() DAAL_C11_OVERRIDE
    {
        return _res ? _res->check(_in, _par, getMethod()) : services::Status(services::ErrorNullResult);
    }
