...
/**
 * <a name="DAAL-CLASS-ALGORITHMS__KDTREE_KNN_CLASSIFICATION__PARTIALMODEL"></a>
 * \brief Partial model trained by the KD-tree based kNN training algorithm in the distributed processing mode
 *
 * \par References
 *      - Parameter class
 *      - \ref kdtree_knn_classification::training::interface1::Distributed "kdtree_knn_classification::training::Distributed"
 */
class DAAL_EXPORT PartialModel : public daal::algorithms::classifier::Model
{
public:
    DECLARE_MODEL(PartialModel, classifier::Model)

     /**
     * Constructs the partial model trained with the KD-tree based kNN algorithm
     * \param[in] nFeatures Number of features in the dataset
     * \DAAL_DEPRECATED_USE{ Model::create }
     */
    PartialModel(size_t nFeatures = 0);
     ...
     * \param[out] stat      Status of the model construction
     */
    static services::SharedPtr<PartialModel> create(size_t nFeatures = 0, services::Status *stat = NULL);

    virtual ~PartialModel();

    class PartialModelImpl;

    /**
     * Returns actual partial model implementation
     * \return Partial model implementation
     */
    const PartialModelImpl * impl() const { return _impl; }

    /**
     * Returns actual partial model implementation
     * \return Partial model implementation
     */
    PartialModelImpl * impl() { return _impl; }
...
protected:
    PartialModel(size_t nFeatures, services::Status &st);

    services::Status serializeImpl(data_management::InputDataArchive   *arch) DAAL_C11_OVERRIDE;
    services::Status deserializeImpl(const data_management::OutputDataArchive *arch) DAAL_C11_OVERRIDE;

private:
    PartialModelImpl *_impl;  /*!< Partial model implementation */
};

typedef services::SharedPtr<Model> ModelPtr;
typedef services::SharedPtr<PartialModel> PartialModelPtr;
typedef services::SharedPtr<const PartialModel> PartialModelConstPtr;
...
using interface1::PartialModelConstPtr;
