...
struct DAAL_EXPORT ParameterBase : public daal::algorithms::classifier::Parameter
{
    ...
    services::Status check() const DAAL_C11_OVERRIDE
    {
        ...
        if(!training.get())
        {
            return services::Status(services::Error::create(services::ErrorNullAuxiliaryAlgorithm, services::ParameterName, trainingStr()));
        }
        if(!prediction.get())
        {
            return services::Status(services::Error::create(services::ErrorNullAuxiliaryAlgorithm, services::ParameterName, predictionStr()));
        }
        return s;
    }
};
