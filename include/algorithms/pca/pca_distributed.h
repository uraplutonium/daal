...
public:
    ...
    */
     ...
     */
    services::Status setPartialResult(const services::SharedPtr<PartialResultImpl<algorithmFPType, correlationDense> >& partialResult)
    {
...
protected:
    ...
    {
        services::Status s = _partialResult->allocate(&input, &parameter, correlationDense);
        _pres = _partialResult.get();
        return s;
    }
...
public:
    ...
    */
     ...
     * \param[in] partialResult    Structure for storing partial results of the PCA algorithm
     */
    services::Status setPartialResult(const services::SharedPtr<PartialResultImpl<algorithmFPType, svdDense> >& partialResult)
    {
        _partialResult = partialResult;
        _pres = _partialResult.get();
        return services::Status();
...
protected:
    ...
    services::Status allocatePartialResult() DAAL_C11_OVERRIDE
    {
        services::Status s = _partialResult->allocate(&input, &parameter, svdDense);
        _pres = _partialResult.get();
        return s;
    }
    services::Status initializePartialResult() DAAL_C11_OVERRIDE{ return services::Status(); }
