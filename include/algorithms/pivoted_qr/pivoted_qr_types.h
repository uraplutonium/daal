...
public:
    ...
    void check(const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        if(inTable->getNumberOfColumns() == 0) { this->_errors->add(services::ErrorIncorrectSizeOfInputNumericTable); return; }
        ...
        {
            if(parameter->permutedColumns->getNumberOfRows() != 1 ||
               parameter->permutedColumns->getNumberOfColumns() != inTable->getNumberOfColumns())
            {
...
public:
    ...
    */
    void check(const daal::algorithms::Input *in, const daal::algorithms::Parameter *par, int method) const DAAL_C11_OVERRIDE
    {
        ...
        services::SharedPtr<data_management::NumericTable> QTable = get(matrixQ);
        services::SharedPtr<data_management::NumericTable> RTable = get(matrixR);
        services::SharedPtr<data_management::NumericTable> PTable = get(permutationMatrix);

        if(QTable->getNumberOfRows() != input->get(data)->getNumberOfRows())
        {
            this->_errors->add(services::ErrorIncorrectNumberOfObservations);
        }
        if(QTable->getNumberOfColumns() != input->get(data)->getNumberOfColumns())
        {
            this->_errors->add(services::ErrorIncorrectNumberOfFeatures);
        }
        if(RTable->getNumberOfRows() != input->get(data)->getNumberOfColumns())
        {
            this->_errors->add(services::ErrorIncorrectNumberOfObservations);
        }
        if(RTable->getNumberOfColumns() != input->get(data)->getNumberOfColumns())
        {
            this->_errors->add(services::ErrorIncorrectNumberOfFeatures);
        }
        if(PTable->getNumberOfRows() != 1)
        {
            this->_errors->add(services::ErrorIncorrectNumberOfObservations);
        }
        if(PTable->getNumberOfColumns() != input->get(data)->getNumberOfColumns())
        {
            this->_errors->add(services::ErrorIncorrectNumberOfFeatures);
        }
    }

    int getSerializationTag() { return SERIALIZATION_PIVOTED_QR_RESULT_ID; }
